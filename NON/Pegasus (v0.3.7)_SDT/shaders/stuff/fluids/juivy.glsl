...
#define SMOOTH_2D_DECCED 1
	...
	#if BEACH_TIMING == 2
		uniform float Worldtime_smooth;
		//float time1 = Worldtime_smooth*.003*OCEAN_SPEED;
	#endif 
...
vec4 beaching_(in vec2 uv, in float h, in float extra_foam) {
	...
	#if BEACH_TIMING == 1
		float time1 = ((frameTimeCounter-floor(frameTimeCounter/72.)*72.)*.05)*OCEAN_SPEED;
	#endif
	#if BEACH_TIMING == 0
		float time1 = worldTime*.003*OCEAN_SPEED;
	#endif	
	#if BEACH_TIMING == 2
		//uniform float Worldtime_smooth;
		float time1 = Worldtime_smooth*.003*OCEAN_SPEED;
...
//normals / height
	...
	*/
			...
			#if WAVE_SHAPE == 2
				vec2 wp = vec2( fnoise3d_j(vec3(uvt+time1*.1,0.)),fnoise3d_j(vec3(uvt-time1*.1,0.)) );
				vec3 ogp = vec3((uvt+wp)*55.,time1*.9)
				#if THIS_IS_DISTANT_HORIZONS == 1
					*.05
				#endif
				;
				vec3 ogp2 = vec3(uvt*100.+time1*1.1,time1*1.)
				#if THIS_IS_DISTANT_HORIZONS == 1
					*.05
				#endif
				;
				vec3 ogp3 = vec3((uvt+wp-time1*.1)*55.,-time1*.1)
				#if THIS_IS_DISTANT_HORIZONS == 1
					*.05
				#endif
				;
