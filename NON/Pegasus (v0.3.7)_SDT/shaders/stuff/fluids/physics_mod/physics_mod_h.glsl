/*
###########################
This code is for interfacing with Physics Mod
It is by Haubna and was provided at the following link specifically for the use of shader compatibility: https://github.com/haubna/PhysicsMod
It is modified from the original version by timetravelbeard
###########################
*/
...
vec3 physics_waveNormal(const in vec2 position, const in vec2 direction, const in float factor, const in float time, out float totalEffect) {
    ...
    totalEffect = left + right + top + bottom;
...
#if FRAG_SHADER == 1
WavePixelData physics_wavePixel(const in vec2 position, const in float factor, const in float iterations, const in float time, out float ripple) {
    ...
    data.normal = physics_waveNormal(position, data.direction, factor, time, ripple);
