...
{
	return fract(sin(fract(sin(p.x*456.+p.y*56.+p.z*741.)*100.)*127.89));
}
	...
	//sky mask
		...
		for(int layer = 0;layer<CLOUD_LAYERS; layer++)
		{
			...
			for(float s = 0.;s < CLOUD_SAMPLES && clouds.a < .99;s++)
			{
				...
				{
					ski*=1.-get_cloud(ray_pos+scale*ss*vec3(0.,1.,0.),layer)*vec3(CLOUD_COLOR_ABSORD_RED, CLOUD_COLOR_ABSORD_GREEN, CLOUD_COLOR_ABSORD_BLUE);
				}
