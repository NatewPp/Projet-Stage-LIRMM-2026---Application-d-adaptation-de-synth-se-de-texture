...
vec4 clouds(in vec2 texcoord,inout float cloud_depth,in vec3 color) {
	...
	//sky mask
		...
		for(int layer = 0;layer<CLOUD_LAYERS; layer++)
		{
			...
			{
				vec3 ray_pos = player + raydir*(s-random3d(frameTimeCounter+vec3(texcoord,s)))*scale;
				
				//slices
				if(s>CLOUD_SAMPLES*.5)
				{
					ray_pos = player + raydir/raydir.y*(s-random3d(frameTimeCounter+vec3(texcoord,s)))*scale;
				}
