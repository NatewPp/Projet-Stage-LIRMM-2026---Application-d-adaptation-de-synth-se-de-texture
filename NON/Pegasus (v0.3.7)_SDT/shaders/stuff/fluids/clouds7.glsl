...
float random3d2(in vec3 p)
{
	return fract(sin(p.x*456.+p.y*56.+p.z*741.)*100.);
	return 
		//fract( p.z*.11 + fractal_noise3d( vec3(p.xy*10.,0.) ));
		fract(sin(p.x*4560.+p.y*560.12+p.z*741.765)*100.432);
}
...
float sky_density = .00;
	...
	float get_cloud(in vec3 p, in int layer)
	{
		...
		p.z*= wisps;
		float c =  clamp(fractal_noise3d(p+layer)*fractal_noise3d(p*.1-layer*5.)
		...
		* (layer >= 1? smooth_noise3d(p*.1/layer+layer*121.):1)
		...
		return  clamp(.5* (c -(.3*(1.-rainStrength)))
...
float get_cloud(in vec3 p, in int layer)
{
	...
	*(layer >= 1? 2.+2.*smooth_noise3d(p+11.*.07) : 4.)//feathery
	...
	return c*.5;//layer != 0 ? c :	p.y<c*CLOUD_LAYER_HEIGHT1?.5:0. ;
...
float cloud_shadow(in vec3 pos) {
		...
		vec3 raydir = 0.* sundir;
		
		float starting_distance = 1./raydir.y;
		
		float scale = .1;
			
		vec2 uv = pos.xz *scale*.1+ raydir.xz*starting_distance+.05* cloud_time*CLOUD_SPEED;
		vec3 player = vec3(uv.x,0.,uv.y)
			//+cameraPosition*scale*.1*vec3(1.,0.,1.)
			;
		
		//return get_cloud(player,1);
		return clamp((fractal_noise3d(player)-.5)*2.+.5*(1.+rainStrength),0.,1.);
		...
		//float sunset_factor = pow(1.-sundir.y,5.);
		vec3 fog_color = mix(sky_color,sky_color*sun_color, sunset_factor*(1.-sun_dot));
		fog_color+=3.*pow(sunset_factor,3.)*sun_color*sun_dot;
		...
		int layer = 0;
			...
			float s = 0.;
				vec3 ray_pos = player + raydir*(s-random3d2(frameTimeCounter+vec3(texcoord,s)))*scale*ray_error;
				
				//slices
				if(s>CLOUD_SAMPLES*.5)
				{
					ray_pos = player + raydir/raydir.y*(s-random3d2(frameTimeCounter+vec3(texcoord,s)))*scale*ray_error;
				}
				
				//get density
				
				return get_cloud(ray_pos,layer);
}
...
vec4 clouds(in vec2 texcoord,inout float cloud_depth,in vec3 color) {
	...
	//sky mask
		...
		#else	
			#if REFLECTIONS_ARE_IN_RAYDIR == 2
				vec4 pos= vec4(texcoord, depth, 1.);
				//vec3 view_pos = pos.xyz;
			#else
				depth = 1.;
		...
		#if REFLECTIONS_ARE_IN_RAYDIR == 2
				vec3 sky_color_og = calcSkyColor_w(raydir);
			#else
				vec3 sky_color_og = sky_color_blurred;
			#endif
		...
		vec3 sky_color = sky_color_og;
			...
			//actual sun
			vec3 sundir_actual = normalize(vec4(gbufferModelViewInverse * vec4(sunPosition.xyz,1.)).xyz);
			float pre_dot_actual_sun = dot(sundir_actual,raydir); 
			float sun_dot_actual = pow((pre_dot_actual_sun+1.)*.5,3.);
			
			//sundir = sundir_actual;//debug
		...
		sky_color=mix(sky_color,sky_color*sun_color, sunset_factor*(1.-sun_dot_actual));
		sky_color+=3.*pow(sunset_factor,3.)*sun_color*sun_dot_actual;
		
		
		
		#if DYNAMIC_CLOUD_COLORATION == 1
			
		
			vec3 absorption_colors = mix( vec3(1.),
			vec3(CLOUD_COLOR_ABSORD_RED, CLOUD_COLOR_ABSORD_GREEN, CLOUD_COLOR_ABSORD_BLUE) 
			,clamp( pow(pre_dot_actual_sun,mix(6.5,1.,sunset_factor) )+sunset_factor,0.,1.) 
			*pow(min(1.,abs(sunAngle-.75)/.25),2.) // not at night
			) ;
			
			vec3 absorption_colors_thick = mix( vec3(1.),
			vec3(CLOUD_COLOR_ABSORD_RED, CLOUD_COLOR_ABSORD_GREEN, CLOUD_COLOR_ABSORD_BLUE) 
			,clamp( pow(pre_dot_actual_sun,mix(1.5,1.,sunset_factor) )+sunset_factor,0.,1.) 
			*pow(min(1.,abs(sunAngle-.75)/.25),2.) // not at night
			) ;
				
			
		#else
			vec3 absorption_colors = vec3(CLOUD_COLOR_ABSORD_RED, CLOUD_COLOR_ABSORD_GREEN, CLOUD_COLOR_ABSORD_BLUE) ;
			vec3 absorption_colors_thick =vec3(CLOUD_COLOR_ABSORD_RED, CLOUD_COLOR_ABSORD_GREEN, CLOUD_COLOR_ABSORD_BLUE) ;
		
		#endif
		
		//float sunset_factor = pow(1.-sundir.y,5.);
		vec3 fog_color = mix(sky_color,sky_color*sun_color, sunset_factor*(1.-sun_dot_actual));
		fog_color+=3.*pow(sunset_factor,3.)*sun_color*sun_dot_actual;
		...
		vec4 clouds= vec4(
			mix(sky_color,
				mix(vec3(MOON_COLOR_R,MOON_COLOR_G,MOON_COLOR_B),vec3(1.), min(1.,pow(abs(sunAngle-.75)/.25,5.) )),
				mix(sun_dot,1.,pow(max(0.,sundir.y),.8))
			),0.); 
		float scale = .1;
		float cloud_shading_amount= .5;
		...
		for(int layer = 0;layer<CLOUD_LAYERS; layer++)
		{
			...
			for(float s = 0.;s < CLOUD_SAMPLES && clouds.a < .99;s++)
			{
				...
				//color by height
				#if HAZY_DAYS == 1
					cloud.rgb = mix(vec3(1.),sky_color,
					.5+.5*sin(worldDay)
					* s / CLOUD_SAMPLES);
				#else
					cloud.rgb = vec3(1.);
				...
				//shading
				vec3 sun_color_here = mix(sun_color,sun_color_hi ,min(1.,ray_pos.y*.5)
				//*pow(1.-sunset_factor,.1)
				
				);
				vec3 light = sun_color_here;
				...
				{
					//move ray
					vec3 ray_s_pos = ray_pos + sundir*.25*(ss-random3d2(frameCounter+vec3(texcoord,ss)))*scale;
					...
					light *=1.- pow(clamp(cloud_shadow,0.,1.),.1+1.-sun_dot)*cloud_shading_amount
						...
						*mix(absorption_colors,absorption_colors_thick,cloud_shadow);
					...
					light.rgb =mix(light.rgb, sun_color_here,highlight.b);
					...
					light += 1.1 *sun_color_here
						...
						*/
						...
						light +=.15*sun_dot2*sun_color_here
				...
				{
					ski*=1.-get_cloud(ray_pos+scale*ss*vec3(0.,1.,0.),layer)*absorption_colors;
				}
				...
				/*
					...
					//sunsset 
					clouds.rgb *= mix(vec3(1.), 1.-absorption_colors,sunset_factor*(1.-max(0.,(sun_dot_actual+1.)/2.))) ;
		...
		*/
		...
		//add distance fog
		 clouds.rgb = mix(clouds.rgb,sky_color,pow(1.-abs(raydir.y),7.)); 
	...
	//color = texcoord.x>.05?color : texcoord.y<sunset_factor?sun_color:color;//debug sunset factor

	return vec4(color, clouds.a); 
