...
const float exposureLuminanceLimit  = 12.5;
...
void main() {
    ...
    if (saturate(qrescoord) == qrescoord) {
        //0.5
        vec4 blur       = textureLod(coltex, qrescoord-1.0*vec2(pixelSize.x, pixelSize.y), 0)/4.0*0.5;
            blur       += textureLod(coltex, qrescoord+1.0*vec2(pixelSize.x, pixelSize.y), 0)/4.0*0.5;
            blur       += textureLod(coltex, qrescoord+1.0*vec2(-pixelSize.x, pixelSize.y), 0)/4.0*0.5;
            blur       += textureLod(coltex, qrescoord+1.0*vec2(pixelSize.x, -pixelSize.y), 0)/4.0*0.5;

        //0.25
            blur       += textureLod(coltex, qrescoord-2.0*vec2(pixelSize.x, 0.0), 0)/2.0*0.125;
            blur       += textureLod(coltex, qrescoord+2.0*vec2(0.0, pixelSize.y), 0)/2.0*0.125;
            blur       += textureLod(coltex, qrescoord+2.0*vec2(-pixelSize.x, 0.0), 0)/2.0*0.125;
            blur       += textureLod(coltex, qrescoord+2.0*vec2(0.0, -pixelSize.y), 0)/2.0*0.125;

        //0.125
            blur       += textureLod(coltex, qrescoord-2.0*vec2(pixelSize.x, pixelSize.y), 0)/4.0*0.125;
            blur       += textureLod(coltex, qrescoord+2.0*vec2(pixelSize.x, pixelSize.y), 0)/4.0*0.125;
            blur       += textureLod(coltex, qrescoord+2.0*vec2(-pixelSize.x, pixelSize.y), 0)/4.0*0.125;
            blur       += textureLod(coltex, qrescoord+2.0*vec2(pixelSize.x, -pixelSize.y), 0)/4.0*0.125;

            blur       += textureLod(coltex, qrescoord, 0)*0.125;
