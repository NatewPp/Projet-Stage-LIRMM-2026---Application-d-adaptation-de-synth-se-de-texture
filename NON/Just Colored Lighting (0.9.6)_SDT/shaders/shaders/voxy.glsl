...
#if IS_VOXY == 1
	#define HAND_HELD_TORCH 0
	#define FLOODFILL_LIGHTING 0
#endif
#include "/noise.glsl"
...
const float dhFarPlane = 16.*3000.;
...
#if (FORCE_VOXY_BUILT_IN_SHADERS == 1 && IS_THE_NETHER !=1 ) || FORCE_VOXY_BUILT_IN_SHADERS == 2
	crash shader
#endif
...
//varyings replaced
 ...
 //lmcoord  = (params.lightMap * gl_MultiTexCoord1).xy;
 //lmcoord=vec2(lmcoord.x,0.);
vec4 glcolor=params.tinting;
vec3 normals_face_world = vec3(uint((params.face>>1)==2), uint((params.face>>1)==0), uint((params.face>>1)==1)) * (float(int(params.face)&1)*2-1);
vec3 normals_face= (gbufferModelView*vec4(normals_face_world,1.)).xyz;
  vec3 viewPos = screenSpaceToViewSpace(
      gl_FragCoord.xyz / vec3(viewWidth, viewHeight, 1.0)
    );
float ipbr_id = float(params.customId);
vec3 foot_pos2 = (gbufferModelViewInverse*vec4(viewPos,1.)).xyz;
vec3 world_pos = foot_pos2+cameraPosition;
vec4 block_centered_relative_pos=vec4(viewPos,0.);
...
vec4 shadowPos = vec4(0.,0.,0., 
	clamp(dot(normals_face,normalize(shadowLightPosition)),0.,1.)
	);
...
//main shader code
	...
	#if IS_THE_NETHER == 1 
		//Voxy only fix
		float cave_light_leak_fix = 0.;
		float cave_light_leak_fix_sky =0.;//
	#endif
	...
	float face_shading = clamp(shadowPos.w,0.,1.);
		...
		#endif
		 color = params.sampledColour;
         color.rgb *= params.tinting.rgb;
		 //gamma correction
	...
	vec3 lava_noise;
			...
			if(abs(ipbr_id - 10032.0)<.5)
			{
						...
						#if PBR == 0
							color.rgb=mix(color.rgb,lava_noise,.5);
							//color.rgb*=mix(lava_noise,vec3(1.),.5);
						#else
							//color.rgb=lava_noise;
							//torch_color.rgb=max(torch_color.rgb,4.*lava_noise);
						#endif
								//	color.rgb=lava_noise;
    ...
    float vanilla_emmisive = abs(ipbr_id-20010.)<=.5? 1.: lmcoord.x>VANILLA_EMMISIVE_THRSHHOLD &&lmcoord.y>VANILLA_EMMISIVE_THRSHHOLD?1.:0.;
	...
	float texture_sss = sss;
		...
		float actual_shadow_distance = shadowDistance;//min(far,shadowDistance);
...
float main_lighting = sunAngle <.5 ? SUN_BRIGHTNESS : MOON_BRIGHTNESS ;
	...
	vec4 shadowLightColor=vec4(1.0);							
		...
		float ao_allowed_light = pow(glcolor.a,VANILLA_AO_EXPONENT);

	#endif	
	
	#if defined IS_IRIS && defined DISTANT_HORIZONS
		//fade out ao into dh
		ao_allowed_light  = mix(ao_allowed_light,1.,clamp(((dist/(far
			//*dh_discard_circle
			))-.8)*5.,0.,1.));
	#endif
	...
	//moved here for voxy when no directional lighting
	...
	torch_color*= mix(1.,BLOCK_LIGHT_BRIGHTNESS_IN_DAYLIGHT,lm.y* pow(max(0.,1.-abs(sunAngle-.25)*4.),.2) );
	...
	#if IS_VOXY == 1
		#if PBR == 0
			torch_color.rgb=max(torch_color.rgb,lava_noise);
		#else
			torch_color.rgb=max(torch_color.rgb,4.*lava_noise);
		#endif
		...
		#endif
		{
				...
				float sdistm = 1./256.;
				float sdist=0.1;
				float shadow_data = face_shading > 0.01?1.:0.;
		...
		,
			...
			main_lighting *sun_color+
				...
				lm.y*sky_color*((1.-ENTITY_LIGHTING_DIRECTIONALITY)
					+ENTITY_LIGHTING_DIRECTIONALITY*clamp(dot(normalize(vec3( ENTITY_LIGHTING_ANGLE_X , ENTITY_LIGHTING_ANGLE_Y , ENTITY_LIGHTING_ANGLE_Z )), normals_face_world.xyz) ,0.,1.) 
					)
				+torch_color
			...
			lm.y*sky_color*
					(normals_face_world.y>.4? 1.: abs(normals_face_world.x)>abs(normals_face_world.z)? .45:.7)
				+torch_color
			#endif
		#endif
...
;
	...
	//final output
	dist_out=dist;
	//if(dist>100.) color = vec4(1.,0.,0.,1.);
//color.rgb=texture(lightmap, params.lightMap).rgb;

//color.rgb=texture(lightmap, lmcoord).rgb;
//color.rgb = vec3(params.lightMap,0.);
//color = vec4(1.,0.,0.,1.);
//color.rgb = vec3(fract(dist_out*.1));
}
