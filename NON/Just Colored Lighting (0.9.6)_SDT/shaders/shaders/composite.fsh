...
#endif
#if VOXEL_AREA == 224
	const float voxelDistance = 224.0;
#endif
#if VOXEL_AREA == 256
...
#else
	return linearize_depth_cpf(texture(depthtex0,uv).r);
#endif
...
{
#if defined IS_IRIS && (defined DISTANT_HORIZONS || defined VOXY ) && BORDERS_IN_DH == 1
	float d = texture(colortex1,uv).x;
	return
//	abs(d-0.) < 0.01 ? 1. :
...
vec3 color = texture(gcolor, texcoord).rgb;
		...
		{
			ao_wide = ( 
			  texture2DLod(colortex1,texcoord,i).x
			+ texture2DLod(colortex1,clamp(texcoord-texelSize*qq         *pow(2,i),0.,1.),i).x
			+ texture2DLod(colortex1,clamp(texcoord-texelSize*vec2(qq,0.)*pow(2.,i),0.,1.),i).x
			+ texture2DLod(colortex1,clamp(texcoord-texelSize*vec2(0.,qq)*pow(2.,i),0.,1.),i).x
			)*.25;
			float oc = ao_dist - ao_wide;
			float oclussion_dist = .1 +0.*ao_dist;
			oc=oc>oclussion_dist?max(0.,oc-(oc-oclussion_dist)):oc;
			oc=ao_dist<2.?oc*(1.+2.*(1.-ao_dist/2.)) : oc;///(ao_dist-5.);
			oc/=1.*pow(ao_dist/far,.8);
			float weight = pow(1.-i/ao_steps,1.0);
			tot_weight+=weight;
			ao+=max(0.,oc)*weight;
		}
		ao =mix( 1., clamp(1.-AO_DARKNESS*ao/tot_weight,0.,1.), AO_DARKNESS);
		//ao*=.5;
		color.rgb*=ao;
		// color=vec3(ao);
	#endif
		...
		float ao_steps=7.;
		float qq = -1.;
		float samplling_width = .1/(1.+ao_dist);
		for(int i = 1;i<ao_steps;i++)
		{
			float r = i*5.+noise(texcoord*5.);
			vec2 spiral=vec2(sin(r),cos(r)) * max(samplling_width,.01);
			ao_wide = ( 
			  texture2DLod(colortex1,texcoord+spiral,0.).x )
			;
			float oc = ao_dist - ao_wide;
			float oclussion_dist = .1 +.0*ao_dist;
			oc=oc>oclussion_dist?max(0.,oc-(oc-oclussion_dist)):oc;
	...
	//	vec2 texelSize = float( BACKGROUND_RESOLUTIION_DIVIDER )/ vec2(viewWidth,viewHeight);
		...
		float dist = material;//texture(colortex1,texcoord).y;
		...
		{}else{
			...
			#if BORDERS >= 2
				vec3 border_color = texture(colortex1, texcoord).rgb;
				color.rgb = 
				mix(
				color.rgb,
				border_color,
...
float l =0.0;
       ...
       // ,5.)
		...
		#endif
        ;
		  //1.-texture(depth,clamp((texcoord-c)*z+c,0.,1.)).r;
...
color.rgb = vec3((	
	...
	#if USE_MANUAL_FINAL_ADJUSTMENTS == 1	
		float brightness = (color.r+color.g+color.b)*.333;
		vec3 hue = color.rgb-brightness;
		color=(((brightness-BTIGHTNESS_CONTRAST_CENTER)*BTIGHTNESS_CONTRAST+BTIGHTNESS_CONTRAST_CENTER)+hue*SATURATION-.5)*CONTRAST+.5+BRIGHTNESS-1.;
...
//color.rgb = vec3(fract( texture(colortex1,texcoord).x *.1 ));//debug
/* DRAWBUFFERS:0 */
