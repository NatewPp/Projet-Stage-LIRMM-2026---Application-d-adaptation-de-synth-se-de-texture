...
#if defined IS_IRIS && (defined DISTANT_HORIZONS || defined VOXY )  && DH_SHADOWS == 1 && DUAL_DISTORT == 1 || FLOODFILL_LIGHTING >= 1 || PIXEL_LOCKED_SHADOW_RES > 0
	#ifndef GBUFFERMODELVIEWINVERSE
uniform mat4 gbufferModelViewInverse;
#define GBUFFERMODELVIEWINVERSE
#endif
...
#if IS_THE_NETHER == 1 || SHADOWS == 7 || (defined IS_IRIS && ((THIS_IS_DISTANT_HORIZONS == 1 && IS_WATER_SHADER == 1 )|| DH_TEXTURE > 0)) || (IS_WATER_SHADER == 1 && FANCY_WATER > 0) || EXTEND_LAVA_PATTERN == 1
	varying vec3 world_pos;
#endif
#if IS_THE_NETHER == 1 || EXTEND_LAVA_PATTERN == 1
...
void main() {
	...
	//lava
			...
			if(abs(ipbr_id - 10032.0)<.5)
			{
						...
						//		.5
							#endif
							lava_noise,clamp((dist-VOXEL_RADIUS*.8)/(VOXEL_RADIUS*.2),0.,1.) );
			}
		#endif
	#endif
	//>lava
