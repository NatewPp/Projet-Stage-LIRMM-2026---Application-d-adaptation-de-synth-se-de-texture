...
void main() {
...
#ifdef DISTANT_HORIZONS
    bool IsDH = landMask(stex(dhDepthTex0).x) && !IsTerrain;
        IsTerrain = IsTerrain || IsDH;
#else
    ...
    if (IsTerrain) {
        ...
        //sceneColor = vec3(1);
        sceneColor *= mix(vec3(textureBicubic(colortex5, uv * 0.5).a), vec3(1.0), DirectLight * 0.75 * float(!IsDH));
