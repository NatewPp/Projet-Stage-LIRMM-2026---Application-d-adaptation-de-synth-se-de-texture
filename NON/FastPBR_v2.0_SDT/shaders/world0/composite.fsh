...
vec2 fogAirDensity(vec3 rPos, float altitude) {
    ...
    return vec2(rm * fogAirScale * fogDensityCoeff.xy * maxFade * fogDensityMult);
}
...
mat2x3 volumetricFog(vec3 scenePos, vec3 sceneDir, bool isSky, float vDotL, float dither, float cave) {
    ...
    uint steps      = uint(stepCoeff * fogAdaptiveSteps) + fogMinSteps;
    ...
    vec3 skylight       = RColorTable.Skylight * cave;
...
void main() {
...
#ifdef DISTANT_HORIZONS
        if (!isTerrain.x) viewPos0 = screenToViewSpace(vec3(uv / ResolutionScale, dhDepth.x), dhProjectionInverse);
#endif
    vec3 scenePos0  = viewToSceneSpace(viewPos0);

    vec3 viewPos1   = screenToViewSpace(vec3(uv / ResolutionScale, sceneDepth.y));
#ifdef DISTANT_HORIZONS
        if (!isTerrain.y) viewPos1 = screenToViewSpace(vec3(uv / ResolutionScale, dhDepth.y), dhProjectionInverse);
#endif
    ...
    bool isSky      = !landMask(sceneDepth.x) && !isDH.x;
