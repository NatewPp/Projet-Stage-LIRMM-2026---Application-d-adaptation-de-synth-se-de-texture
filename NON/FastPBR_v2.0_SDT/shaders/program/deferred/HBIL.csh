...
float getDistantSSAO(float depth, vec2 coord, float dither) {
    const uint steps = 4;
    const float baseRadius = 6.0;

    bool hand       = depth < 0.56;
        depth       = depthLinearDH(depth);
    ...
    const float maxOcclusionDist    = pi*4;
    const float anibleedExp         = 1.0;

    for (uint i = 0; i < steps; ++i) {
		vec2 offset = offsetDist(currStep) * scale;
		float mult  = (0.7 / baseRadius) * (dhFarPlane - dhNearPlane);

		float sampleDepth = depthLinearDH(texture(dhDepthTex0, (coord + offset) * ResolutionScale).r);
		float sample0 = (depth - sampleDepth) * mult;
        float antiBleed = 1.0 - rcp(1.0 + max0(distance(sampleDepth, depth) * dhFarPlane - maxOcclusionDist) * anibleedExp);
		float angle = mix(clamp(0.5 - sample0, 0.0, 1.0), 0.5, antiBleed);
		float dist  = mix(clamp(0.25 * sample0 - 1.0, 0.0, 1.0), 0.5, antiBleed);

		sampleDepth = depthLinearDH(texture(dhDepthTex0, (coord - offset) * ResolutionScale).r);
		sample0     = (depth - sampleDepth) * mult;
        antiBleed   = 1.0 - rcp(1.0 + max0(distance(sampleDepth, depth) * dhFarPlane - maxOcclusionDist) * anibleedExp);
        angle      += mix(clamp(0.5 - sample0, 0.0, 1.0), 0.5, antiBleed);
        dist       += mix(clamp(0.25 * sample0 - 1.0, 0.0, 1.0), 0.5, antiBleed);
	...
	return sqr(saturate(ao)) * 0.98 + 0.02;
}
#endif
...
vec4 HorizonSearch(inout float CosTheta, vec3 ScreenPosition, vec3 ViewPosition, vec3 ViewSlice, vec3 ViewDir, vec2 SliceNormal, float Radius, float Dither) {
    ...
    for (uint i = 0; i < HBIL_HORIZON_SAMPLES; ++i, RayPosition += RayStep) {
        ...
        float DepthSample = texelFetch(depthtex0, ivec2(SamplePosition * viewSize * ResolutionScale - 0.5), 0).x;
        ...
        vec3 Radiance = texelFetch(colortex0, ivec2(saturate(SamplePosition) * viewSize * ResolutionScale - 0.5), 0).rgb * Falloff;
...
void main() {
...
#ifdef DISTANT_HORIZONS
    float sceneDepthDH  = texelFetch(dhDepthTex0, ViewPixel, 0).x;
#endif
    ...
    if (landMask(sceneDepth) && saturate(ViewUV) == ViewUV) {
        ...
        #endif
#ifdef DISTANT_HORIZONS
    } else if (landMask(sceneDepthDH) && saturate(ViewUV) == ViewUV) {
        Irradiance.a = getDistantSSAO(sceneDepthDH, saturate(ViewUV), ditherBluenoise());
#endif
    }
    #endif
