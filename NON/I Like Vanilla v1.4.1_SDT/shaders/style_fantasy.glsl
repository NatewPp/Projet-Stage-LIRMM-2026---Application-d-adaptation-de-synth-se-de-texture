#if FOLIAGE_NOISE_ENABLED == -1
#undef FOLIAGE_NOISE_ENABLED
										#define FOLIAGE_NOISE_ENABLED 1
#endif
#if FOLIAGE_NOISE_AMOUNT == -1
#undef FOLIAGE_NOISE_AMOUNT
										#define FOLIAGE_NOISE_AMOUNT 0.7
#endif
#if LAVA_NOISE_ENABLED == -1
#undef LAVA_NOISE_ENABLED
										#define LAVA_NOISE_ENABLED 1
#endif
#if LAVA_NOISE_AMOUNT_OVERWORLD == -1
#undef LAVA_NOISE_AMOUNT_OVERWORLD
										#define LAVA_NOISE_AMOUNT_OVERWORLD 0.75
#endif
#if LAVA_NOISE_AMOUNT_NETHER == -1
#undef LAVA_NOISE_AMOUNT_NETHER
										#define LAVA_NOISE_AMOUNT_NETHER 1.1
#endif
#if LAVA_NOISE_AMOUNT_END == -1
#undef LAVA_NOISE_AMOUNT_END
										#define LAVA_NOISE_AMOUNT_END 0.5
#endif
#if FANCY_NETHER_PORTAL_ENABLED == -1
#undef FANCY_NETHER_PORTAL_ENABLED
										#define FANCY_NETHER_PORTAL_ENABLED 1
#endif
#if FANCY_END_PORTAL_ENABLED == -1
#undef FANCY_END_PORTAL_ENABLED
										#define FANCY_END_PORTAL_ENABLED 2
#endif
#if ENCHANTMENT_GLINT_STRENGTH == -1
#undef ENCHANTMENT_GLINT_STRENGTH
										#define ENCHANTMENT_GLINT_STRENGTH 1.2
#endif





#if PBR_NORMALS_AMOUNT == -1
#undef PBR_NORMALS_AMOUNT
										#define PBR_NORMALS_AMOUNT 0.8
#endif





#if GLOWING_ORES_STRENGTH_OVERWORLD == -1
#undef GLOWING_ORES_STRENGTH_OVERWORLD
										#define GLOWING_ORES_STRENGTH_OVERWORLD 0.4
#endif
#if GLOWING_ORES_STRENGTH_NETHER == -1
#undef GLOWING_ORES_STRENGTH_NETHER
										#define GLOWING_ORES_STRENGTH_NETHER 0.9
#endif
#if GLOWING_ORES_STRENGTH_END == -1
#undef GLOWING_ORES_STRENGTH_END
										#define GLOWING_ORES_STRENGTH_END 0.75
#endif
#if GLOWING_STEMS_STRENGTH == -1
#undef GLOWING_STEMS_STRENGTH
										#define GLOWING_STEMS_STRENGTH 0.75
#endif





#if POM_DEPTH == -1
#undef POM_DEPTH
										#define POM_DEPTH 1.0
#endif





#if SHADOWS_NOISE == -1
#undef SHADOWS_NOISE
										#define SHADOWS_NOISE 0.6
#endif
#if PIXELATED_SHADOWS == -1
#undef PIXELATED_SHADOWS
										#define PIXELATED_SHADOWS 0
#endif
#if PIXELATED_SHADOWS_SOFTNESS == -1
#undef PIXELATED_SHADOWS_SOFTNESS
										#define PIXELATED_SHADOWS_SOFTNESS 1.0
#endif
#if EXCLUDE_FOLIAGE == -1
#undef EXCLUDE_FOLIAGE
										#define EXCLUDE_FOLIAGE 0
#endif





#if WATER_CAUSTICS_SIZE == -1
#undef WATER_CAUSTICS_SIZE
										#define WATER_CAUSTICS_SIZE 1.0
#endif
#if WATER_CAUSTICS_HORIZONTAL_STRETCH == -1
#undef WATER_CAUSTICS_HORIZONTAL_STRETCH
										#define WATER_CAUSTICS_HORIZONTAL_STRETCH 2.0
#endif
#if WATER_CAUSTICS_SPEED == -1
#undef WATER_CAUSTICS_SPEED
										#define WATER_CAUSTICS_SPEED 1.0
#endif
#if WATER_CAUSTICS_THICKNESS == -1
#undef WATER_CAUSTICS_THICKNESS
										#define WATER_CAUSTICS_THICKNESS 1.0
#endif
#if WATER_CAUSTICS_DARK_RED == -1
#undef WATER_CAUSTICS_DARK_RED
										#define WATER_CAUSTICS_DARK_RED 0.35
#endif
#if WATER_CAUSTICS_DARK_GREEN == -1
#undef WATER_CAUSTICS_DARK_GREEN
										#define WATER_CAUSTICS_DARK_GREEN 0.45
#endif
#if WATER_CAUSTICS_DARK_BLUE == -1
#undef WATER_CAUSTICS_DARK_BLUE
										#define WATER_CAUSTICS_DARK_BLUE 0.75
#endif
#if WATER_CAUSTICS_DARK_BRIGHTNESS == -1
#undef WATER_CAUSTICS_DARK_BRIGHTNESS
										#define WATER_CAUSTICS_DARK_BRIGHTNESS 1.0
#endif
#if WATER_CAUSTICS_BRIGHT_RED == -1
#undef WATER_CAUSTICS_BRIGHT_RED
										#define WATER_CAUSTICS_BRIGHT_RED 0.7
#endif
#if WATER_CAUSTICS_BRIGHT_GREEN == -1
#undef WATER_CAUSTICS_BRIGHT_GREEN
										#define WATER_CAUSTICS_BRIGHT_GREEN 0.9
#endif
#if WATER_CAUSTICS_BRIGHT_BLUE == -1
#undef WATER_CAUSTICS_BRIGHT_BLUE
										#define WATER_CAUSTICS_BRIGHT_BLUE 1.2
#endif
#if WATER_CAUSTICS_BRIGHT_BRIGHTNESS == -1
#undef WATER_CAUSTICS_BRIGHT_BRIGHTNESS
										#define WATER_CAUSTICS_BRIGHT_BRIGHTNESS 1.0
#endif






#if VANILLA_AO_BRIGHT == -1
#undef VANILLA_AO_BRIGHT
										#define VANILLA_AO_BRIGHT 1.2
#endif
#if VANILLA_AO_DARK == -1
#undef VANILLA_AO_DARK
										#define VANILLA_AO_DARK 1.1
#endif
#if SIDE_SHADING_BRIGHT == -1
#undef SIDE_SHADING_BRIGHT
										#define SIDE_SHADING_BRIGHT 0.5
#endif
#if SIDE_SHADING_DARK == -1
#undef SIDE_SHADING_DARK
										#define SIDE_SHADING_DARK 0.4
#endif
#if OVERRIDE_FOLIAGE_NORMALS == -1
#undef OVERRIDE_FOLIAGE_NORMALS
										#define OVERRIDE_FOLIAGE_NORMALS 1
#endif
#if NIGHT_SATURATION_DECREASE == -1
#undef NIGHT_SATURATION_DECREASE
										#define NIGHT_SATURATION_DECREASE 1.0
#endif
#if WEATHER_BRIGHTNESS_MULT_DAY == -1
#undef WEATHER_BRIGHTNESS_MULT_DAY
										#define WEATHER_BRIGHTNESS_MULT_DAY 0.35
#endif
#if WEATHER_BRIGHTNESS_MULT_NIGHT == -1
#undef WEATHER_BRIGHTNESS_MULT_NIGHT
										#define WEATHER_BRIGHTNESS_MULT_NIGHT 0.35
#endif
#if BRIGHTNESS_TWEAKS_STRENGTH == -1
#undef BRIGHTNESS_TWEAKS_STRENGTH
										#define BRIGHTNESS_TWEAKS_STRENGTH 0.7
#endif





#if SUNLIGHT_CEL_AMOUNT == -1
#undef SUNLIGHT_CEL_AMOUNT
										#define SUNLIGHT_CEL_AMOUNT 0
#endif
#if AMBIENT_CEL_AMOUNT == -1
#undef AMBIENT_CEL_AMOUNT
										#define AMBIENT_CEL_AMOUNT 0
#endif
#if BLOCKLIGHT_CEL_AMOUNT == -1
#undef BLOCKLIGHT_CEL_AMOUNT
										#define BLOCKLIGHT_CEL_AMOUNT 0
#endif





#if LIGHTNING_BRIGHTNESS == -1
#undef LIGHTNING_BRIGHTNESS
										#define LIGHTNING_BRIGHTNESS 0.8
#endif





#if SKYLIGHT_DAY_RED == -1
#undef SKYLIGHT_DAY_RED
										#define SKYLIGHT_DAY_RED 1.0
#endif
#if SKYLIGHT_DAY_GREEN == -1
#undef SKYLIGHT_DAY_GREEN
										#define SKYLIGHT_DAY_GREEN 0.9
#endif
#if SKYLIGHT_DAY_BLUE == -1
#undef SKYLIGHT_DAY_BLUE
										#define SKYLIGHT_DAY_BLUE 0.75
#endif
#if SKYLIGHT_DAY_BRIGHTNESS == -1
#undef SKYLIGHT_DAY_BRIGHTNESS
										#define SKYLIGHT_DAY_BRIGHTNESS 1.4
#endif
#if AMBIENT_DAY_RED == -1
#undef AMBIENT_DAY_RED
										#define AMBIENT_DAY_RED 0.75
#endif
#if AMBIENT_DAY_GREEN == -1
#undef AMBIENT_DAY_GREEN
										#define AMBIENT_DAY_GREEN 0.8
#endif
#if AMBIENT_DAY_BLUE == -1
#undef AMBIENT_DAY_BLUE
										#define AMBIENT_DAY_BLUE 1.0
#endif
#if AMBIENT_DAY_BRIGHTNESS == -1
#undef AMBIENT_DAY_BRIGHTNESS
										#define AMBIENT_DAY_BRIGHTNESS 0.75
#endif





#if SKYLIGHT_NIGHT_RED == -1
#undef SKYLIGHT_NIGHT_RED
										#define SKYLIGHT_NIGHT_RED 0.45
#endif
#if SKYLIGHT_NIGHT_GREEN == -1
#undef SKYLIGHT_NIGHT_GREEN
										#define SKYLIGHT_NIGHT_GREEN 0.75
#endif
#if SKYLIGHT_NIGHT_BLUE == -1
#undef SKYLIGHT_NIGHT_BLUE
										#define SKYLIGHT_NIGHT_BLUE 1.3
#endif
#if SKYLIGHT_NIGHT_BRIGHTNESS == -1
#undef SKYLIGHT_NIGHT_BRIGHTNESS
										#define SKYLIGHT_NIGHT_BRIGHTNESS 0.26
#endif
#if AMBIENT_NIGHT_RED == -1
#undef AMBIENT_NIGHT_RED
										#define AMBIENT_NIGHT_RED 0.45
#endif
#if AMBIENT_NIGHT_GREEN == -1
#undef AMBIENT_NIGHT_GREEN
										#define AMBIENT_NIGHT_GREEN 0.75
#endif
#if AMBIENT_NIGHT_BLUE == -1
#undef AMBIENT_NIGHT_BLUE
										#define AMBIENT_NIGHT_BLUE 1.4
#endif
#if AMBIENT_NIGHT_BRIGHTNESS == -1
#undef AMBIENT_NIGHT_BRIGHTNESS
										#define AMBIENT_NIGHT_BRIGHTNESS 0.2
#endif
#if MOON_PHASE_BRIGHTNESS_MULT_1 == -1
#undef MOON_PHASE_BRIGHTNESS_MULT_1
										#define MOON_PHASE_BRIGHTNESS_MULT_1 1.2
#endif
#if MOON_PHASE_BRIGHTNESS_MULT_2 == -1
#undef MOON_PHASE_BRIGHTNESS_MULT_2
										#define MOON_PHASE_BRIGHTNESS_MULT_2 1.1
#endif
#if MOON_PHASE_BRIGHTNESS_MULT_3 == -1
#undef MOON_PHASE_BRIGHTNESS_MULT_3
										#define MOON_PHASE_BRIGHTNESS_MULT_3 1.0
#endif
#if MOON_PHASE_BRIGHTNESS_MULT_4 == -1
#undef MOON_PHASE_BRIGHTNESS_MULT_4
										#define MOON_PHASE_BRIGHTNESS_MULT_4 0.9
#endif
#if MOON_PHASE_BRIGHTNESS_MULT_5 == -1
#undef MOON_PHASE_BRIGHTNESS_MULT_5
										#define MOON_PHASE_BRIGHTNESS_MULT_5 0.8
#endif





#if SKYLIGHT_SUNRISE_RED == -1
#undef SKYLIGHT_SUNRISE_RED
										#define SKYLIGHT_SUNRISE_RED 1.3
#endif
#if SKYLIGHT_SUNRISE_GREEN == -1
#undef SKYLIGHT_SUNRISE_GREEN
										#define SKYLIGHT_SUNRISE_GREEN 0.6
#endif
#if SKYLIGHT_SUNRISE_BLUE == -1
#undef SKYLIGHT_SUNRISE_BLUE
										#define SKYLIGHT_SUNRISE_BLUE 0.15
#endif
#if SKYLIGHT_SUNRISE_BRIGHTNESS == -1
#undef SKYLIGHT_SUNRISE_BRIGHTNESS
										#define SKYLIGHT_SUNRISE_BRIGHTNESS 1.4
#endif
#if AMBIENT_SUNRISE_RED == -1
#undef AMBIENT_SUNRISE_RED
										#define AMBIENT_SUNRISE_RED 1.0
#endif
#if AMBIENT_SUNRISE_GREEN == -1
#undef AMBIENT_SUNRISE_GREEN
										#define AMBIENT_SUNRISE_GREEN 0.6
#endif
#if AMBIENT_SUNRISE_BLUE == -1
#undef AMBIENT_SUNRISE_BLUE
										#define AMBIENT_SUNRISE_BLUE 0.8
#endif
#if AMBIENT_SUNRISE_BRIGHTNESS == -1
#undef AMBIENT_SUNRISE_BRIGHTNESS
										#define AMBIENT_SUNRISE_BRIGHTNESS 0.65
#endif





#if SKYLIGHT_SUNSET_RED == -1
#undef SKYLIGHT_SUNSET_RED
										#define SKYLIGHT_SUNSET_RED 1.25
#endif
#if SKYLIGHT_SUNSET_GREEN == -1
#undef SKYLIGHT_SUNSET_GREEN
										#define SKYLIGHT_SUNSET_GREEN 0.7
#endif
#if SKYLIGHT_SUNSET_BLUE == -1
#undef SKYLIGHT_SUNSET_BLUE
										#define SKYLIGHT_SUNSET_BLUE 0.15
#endif
#if SKYLIGHT_SUNSET_BRIGHTNESS == -1
#undef SKYLIGHT_SUNSET_BRIGHTNESS
										#define SKYLIGHT_SUNSET_BRIGHTNESS 1.4
#endif
#if AMBIENT_SUNSET_RED == -1
#undef AMBIENT_SUNSET_RED
										#define AMBIENT_SUNSET_RED 1.0
#endif
#if AMBIENT_SUNSET_GREEN == -1
#undef AMBIENT_SUNSET_GREEN
										#define AMBIENT_SUNSET_GREEN 0.65
#endif
#if AMBIENT_SUNSET_BLUE == -1
#undef AMBIENT_SUNSET_BLUE
										#define AMBIENT_SUNSET_BLUE 0.8
#endif
#if AMBIENT_SUNSET_BRIGHTNESS == -1
#undef AMBIENT_SUNSET_BRIGHTNESS
										#define AMBIENT_SUNSET_BRIGHTNESS 0.65
#endif





#if BLOCK_RED_DARK == -1
#undef BLOCK_RED_DARK
										#define BLOCK_RED_DARK 1.1
#endif
#if BLOCK_GREEN_DARK == -1
#undef BLOCK_GREEN_DARK
										#define BLOCK_GREEN_DARK 0.65
#endif
#if BLOCK_BLUE_DARK == -1
#undef BLOCK_BLUE_DARK
										#define BLOCK_BLUE_DARK 0.35
#endif
#if BLOCK_RED_BRIGHT == -1
#undef BLOCK_RED_BRIGHT
										#define BLOCK_RED_BRIGHT 1.0
#endif
#if BLOCK_GREEN_BRIGHT == -1
#undef BLOCK_GREEN_BRIGHT
										#define BLOCK_GREEN_BRIGHT 0.7
#endif
#if BLOCK_BLUE_BRIGHT == -1
#undef BLOCK_BLUE_BRIGHT
										#define BLOCK_BLUE_BRIGHT 0.5
#endif
#if BLOCK_BRIGHTNESS == -1
#undef BLOCK_BRIGHTNESS
										#define BLOCK_BRIGHTNESS 0.8
#endif
#if BLOCK_BRIGHTNESS_CURVE == -1
#undef BLOCK_BRIGHTNESS_CURVE
										#define BLOCK_BRIGHTNESS_CURVE 1
#endif
#if BLOCK_BRIGHTNESS_NIGHT_MULT == -1
#undef BLOCK_BRIGHTNESS_NIGHT_MULT
										#define BLOCK_BRIGHTNESS_NIGHT_MULT 1.6
#endif
#if BLOCKLIGHT_FLICKERING_ENABLED == -1
#undef BLOCKLIGHT_FLICKERING_ENABLED
										#define BLOCKLIGHT_FLICKERING_ENABLED 1
#endif
#if BLOCKLIGHT_FLICKERING_AMOUNT == -1
#undef BLOCKLIGHT_FLICKERING_AMOUNT
										#define BLOCKLIGHT_FLICKERING_AMOUNT 1.0
#endif





#if CAVE_AMBIENT_RED == -1
#undef CAVE_AMBIENT_RED
										#define CAVE_AMBIENT_RED 0.96
#endif
#if CAVE_AMBIENT_GREEN == -1
#undef CAVE_AMBIENT_GREEN
										#define CAVE_AMBIENT_GREEN 0.7
#endif
#if CAVE_AMBIENT_BLUE == -1
#undef CAVE_AMBIENT_BLUE
										#define CAVE_AMBIENT_BLUE 0.65
#endif
#if CAVE_AMBIENT_BRIGHTNESS == -1
#undef CAVE_AMBIENT_BRIGHTNESS
										#define CAVE_AMBIENT_BRIGHTNESS 0.2
#endif





#if NETHER_AMBIENT_RED == -1
#undef NETHER_AMBIENT_RED
										#define NETHER_AMBIENT_RED 1.0
#endif
#if NETHER_AMBIENT_GREEN == -1
#undef NETHER_AMBIENT_GREEN
										#define NETHER_AMBIENT_GREEN 0.55
#endif
#if NETHER_AMBIENT_BLUE == -1
#undef NETHER_AMBIENT_BLUE
										#define NETHER_AMBIENT_BLUE 0.2
#endif
#if NETHER_AMBIENT_BRIGHTNESS == -1
#undef NETHER_AMBIENT_BRIGHTNESS
										#define NETHER_AMBIENT_BRIGHTNESS 0.2
#endif
#if NETHER_BLOCKLIGHT_RED_MULT == -1
#undef NETHER_BLOCKLIGHT_RED_MULT
										#define NETHER_BLOCKLIGHT_RED_MULT 1.0
#endif
#if NETHER_BLOCKLIGHT_GREEN_MULT == -1
#undef NETHER_BLOCKLIGHT_GREEN_MULT
										#define NETHER_BLOCKLIGHT_GREEN_MULT 0.6
#endif
#if NETHER_BLOCKLIGHT_BLUE_MULT == -1
#undef NETHER_BLOCKLIGHT_BLUE_MULT
										#define NETHER_BLOCKLIGHT_BLUE_MULT 0.3
#endif
#if NETHER_BLOCKLIGHT_BRIGHTNESS_MULT == -1
#undef NETHER_BLOCKLIGHT_BRIGHTNESS_MULT
										#define NETHER_BLOCKLIGHT_BRIGHTNESS_MULT 2.0
#endif





#if END_SKYLIGHT_RED == -1
#undef END_SKYLIGHT_RED
										#define END_SKYLIGHT_RED 0.9
#endif
#if END_SKYLIGHT_GREEN == -1
#undef END_SKYLIGHT_GREEN
										#define END_SKYLIGHT_GREEN 0.55
#endif
#if END_SKYLIGHT_BLUE == -1
#undef END_SKYLIGHT_BLUE
										#define END_SKYLIGHT_BLUE 1.15
#endif
#if END_SKYLIGHT_BRIGHTNESS == -1
#undef END_SKYLIGHT_BRIGHTNESS
										#define END_SKYLIGHT_BRIGHTNESS 0.5
#endif
#if END_AMBIENT_RED == -1
#undef END_AMBIENT_RED
										#define END_AMBIENT_RED 0.85
#endif
#if END_AMBIENT_GREEN == -1
#undef END_AMBIENT_GREEN
										#define END_AMBIENT_GREEN 0.55
#endif
#if END_AMBIENT_BLUE == -1
#undef END_AMBIENT_BLUE
										#define END_AMBIENT_BLUE 1.15
#endif
#if END_AMBIENT_BRIGHTNESS == -1
#undef END_AMBIENT_BRIGHTNESS
										#define END_AMBIENT_BRIGHTNESS 0.4
#endif





#if HANDHELD_LIGHT_ENABLED == -1
#undef HANDHELD_LIGHT_ENABLED
										#define HANDHELD_LIGHT_ENABLED 1
#endif
#if HANDHELD_LIGHT_DISTANCE == -1
#undef HANDHELD_LIGHT_DISTANCE
										#define HANDHELD_LIGHT_DISTANCE 15
#endif
#if HANDHELD_LIGHT_BRIGHTNESS == -1
#undef HANDHELD_LIGHT_BRIGHTNESS
										#define HANDHELD_LIGHT_BRIGHTNESS 1.0
#endif
#if HANDHELD_LIGHT_REALISM == -1
#undef HANDHELD_LIGHT_REALISM
										#define HANDHELD_LIGHT_REALISM 0.5
#endif





#if NIGHT_VISION_BRIGHTNESS == -1
#undef NIGHT_VISION_BRIGHTNESS
										#define NIGHT_VISION_BRIGHTNESS 0.6
#endif
#if NIGHT_VISION_GREEN_AMOUNT == -1
#undef NIGHT_VISION_GREEN_AMOUNT
										#define NIGHT_VISION_GREEN_AMOUNT 0.5
#endif
#if TEXTURE_CONTRAST == -1
#undef TEXTURE_CONTRAST
										#define TEXTURE_CONTRAST 0.25
#endif
#if TEXTURE_CONTRAST_2 == -1
#undef TEXTURE_CONTRAST_2
										#define TEXTURE_CONTRAST_2 0.0
#endif
#if FOLIAGE_SATURATION == -1
#undef FOLIAGE_SATURATION
										#define FOLIAGE_SATURATION 1.0
#endif
#if FOLIAGE_TINT_RED == -1
#undef FOLIAGE_TINT_RED
										#define FOLIAGE_TINT_RED 1.0
#endif
#if FOLIAGE_TINT_GREEN == -1
#undef FOLIAGE_TINT_GREEN
										#define FOLIAGE_TINT_GREEN 1.0
#endif
#if FOLIAGE_TINT_BLUE == -1
#undef FOLIAGE_TINT_BLUE
										#define FOLIAGE_TINT_BLUE 1.0
#endif
#if SNOWY_TWEAKS_ENABLED == -1
#undef SNOWY_TWEAKS_ENABLED
										#define SNOWY_TWEAKS_ENABLED 1
#endif





#if HBD_ENABLED == -1
#undef HBD_ENABLED
										#define HBD_ENABLED 1
#endif
#if HBD_STRENGTH == -1
#undef HBD_STRENGTH
										#define HBD_STRENGTH 0.25
#endif
#if HBD_SCALE == -1
#undef HBD_SCALE
										#define HBD_SCALE 60
#endif





#if TAA_JITTER_AMOUNT == -1
#undef TAA_JITTER_AMOUNT
										#define TAA_JITTER_AMOUNT 0.95
#endif
#if OUTLINES_ENABLED == -1
#undef OUTLINES_ENABLED
										#define OUTLINES_ENABLED 0
#endif





#if BLOOM_STYLE == -1
#undef BLOOM_STYLE
										#define BLOOM_STYLE 1
#endif
#if BLOOM_AMOUNT == -1
#undef BLOOM_AMOUNT
										#define BLOOM_AMOUNT 1.0
#endif
#if BLOOM_NETHER_AMOUNT == -1
#undef BLOOM_NETHER_AMOUNT
										#define BLOOM_NETHER_AMOUNT 1.5
#endif
#if BLOOM_END_AMOUNT == -1
#undef BLOOM_END_AMOUNT
										#define BLOOM_END_AMOUNT 0.6
#endif
#if BLOOM_SIZE == -1
#undef BLOOM_SIZE
										#define BLOOM_SIZE 1.2
#endif
#if BLOOM_LOW_CUTOFF == -1
#undef BLOOM_LOW_CUTOFF
										#define BLOOM_LOW_CUTOFF 0.3
#endif
#if BLOOM_HIGH_CUTOFF == -1
#undef BLOOM_HIGH_CUTOFF
										#define BLOOM_HIGH_CUTOFF 1.0
#endif
#if BLOOM_DETECT_TINT_RED == -1
#undef BLOOM_DETECT_TINT_RED
										#define BLOOM_DETECT_TINT_RED 1.0
#endif
#if BLOOM_DETECT_TINT_GREEN == -1
#undef BLOOM_DETECT_TINT_GREEN
										#define BLOOM_DETECT_TINT_GREEN 1.0
#endif
#if BLOOM_DETECT_TINT_BLUE == -1
#undef BLOOM_DETECT_TINT_BLUE
										#define BLOOM_DETECT_TINT_BLUE 1.0
#endif





#if AO_AMOUNT_LIT == -1
#undef AO_AMOUNT_LIT
										#define AO_AMOUNT_LIT 0.7
#endif
#if AO_AMOUNT_UNLIT == -1
#undef AO_AMOUNT_UNLIT
										#define AO_AMOUNT_UNLIT 0.8
#endif
#if AO_SIZE == -1
#undef AO_SIZE
										#define AO_SIZE 1.1
#endif





#if REFLECTION_FRESNEL == -1
#undef REFLECTION_FRESNEL
										#define REFLECTION_FRESNEL 0.5
#endif
#if BLOCK_REFLECTION_AMOUNT_SURFACE == -1
#undef BLOCK_REFLECTION_AMOUNT_SURFACE
										#define BLOCK_REFLECTION_AMOUNT_SURFACE 0.45
#endif
#if BLOCK_REFLECTION_AMOUNT_UNDERGROUND == -1
#undef BLOCK_REFLECTION_AMOUNT_UNDERGROUND
										#define BLOCK_REFLECTION_AMOUNT_UNDERGROUND 0.45
#endif
#if WATER_REFLECTION_AMOUNT_SURFACE == -1
#undef WATER_REFLECTION_AMOUNT_SURFACE
										#define WATER_REFLECTION_AMOUNT_SURFACE 0.5
#endif
#if WATER_REFLECTION_AMOUNT_UNDERGROUND == -1
#undef WATER_REFLECTION_AMOUNT_UNDERGROUND
										#define WATER_REFLECTION_AMOUNT_UNDERGROUND 0.5
#endif
#if REFLECTIONS_BRIGHTNESS == -1
#undef REFLECTIONS_BRIGHTNESS
										#define REFLECTIONS_BRIGHTNESS 1.25
#endif
//#if RAIN_REFLECTION_AMOUNT == -1
//#undef RAIN_REFLECTION_AMOUNT
//										#define RAIN_REFLECTION_AMOUNT 0.7
//#endif
//#if RAIN_REFLECTION_COVERAGE == -1
//#undef RAIN_REFLECTION_COVERAGE
//										#define RAIN_REFLECTION_COVERAGE 0.6
//#endif
//#if RAIN_REFLECTION_MIN == -1
//#undef RAIN_REFLECTION_MIN
//										#define RAIN_REFLECTION_MIN 0.5
//#endif
//#if RAIN_REFLECTION_SLOPE == -1
//#undef RAIN_REFLECTION_SLOPE
//										#define RAIN_REFLECTION_SLOPE 1.5
//#endif





#if DEPTH_SUNRAYS_ENABLED == -1
#undef DEPTH_SUNRAYS_ENABLED
										#define DEPTH_SUNRAYS_ENABLED 1
#endif
#if VOL_SUNRAYS_ENABLED == -1
#undef VOL_SUNRAYS_ENABLED
										#define VOL_SUNRAYS_ENABLED 1
#endif
#if SUNRAYS_WEATHER_MULT == -1
#undef SUNRAYS_WEATHER_MULT
										#define SUNRAYS_WEATHER_MULT 0.03
#endif
#if SUNRAYS_AMOUNT_DAY == -1
#undef SUNRAYS_AMOUNT_DAY
										#define SUNRAYS_AMOUNT_DAY 0.45
#endif
#if SUNRAYS_AMOUNT_NIGHT == -1
#undef SUNRAYS_AMOUNT_NIGHT
										#define SUNRAYS_AMOUNT_NIGHT 0.25
#endif
#if SUNRAYS_INCREASE_SUNRISE == -1
#undef SUNRAYS_INCREASE_SUNRISE
										#define SUNRAYS_INCREASE_SUNRISE 2.8
#endif
#if SUNRAYS_INCREASE_SUNSET == -1
#undef SUNRAYS_INCREASE_SUNSET
										#define SUNRAYS_INCREASE_SUNSET 2.8
#endif
#if SUNRAYS_SUN_RED == -1
#undef SUNRAYS_SUN_RED
										#define SUNRAYS_SUN_RED 1.1
#endif
#if SUNRAYS_SUN_GREEN == -1
#undef SUNRAYS_SUN_GREEN
										#define SUNRAYS_SUN_GREEN 0.3
#endif
#if SUNRAYS_SUN_BLUE == -1
#undef SUNRAYS_SUN_BLUE
										#define SUNRAYS_SUN_BLUE 0.1
#endif
#if SUNRAYS_MOON_RED == -1
#undef SUNRAYS_MOON_RED
										#define SUNRAYS_MOON_RED 0.0
#endif
#if SUNRAYS_MOON_GREEN == -1
#undef SUNRAYS_MOON_GREEN
										#define SUNRAYS_MOON_GREEN 0.45
#endif
#if SUNRAYS_MOON_BLUE == -1
#undef SUNRAYS_MOON_BLUE
										#define SUNRAYS_MOON_BLUE 1.0
#endif





#if DEPTH_SUNRAYS_STYLE == -1
#undef DEPTH_SUNRAYS_STYLE
										#define DEPTH_SUNRAYS_STYLE 1
#endif
#if SUNRAYS_FLICKERING_FIX == -1
#undef SUNRAYS_FLICKERING_FIX
										#define SUNRAYS_FLICKERING_FIX 0
#endif





#if SUNRAYS_UNDERGROUND_MULT == -1
#undef SUNRAYS_UNDERGROUND_MULT
										#define SUNRAYS_UNDERGROUND_MULT 3.0
#endif
#if SUNRAYS_AMOUNT_MAX_DAY == -1
#undef SUNRAYS_AMOUNT_MAX_DAY
										#define SUNRAYS_AMOUNT_MAX_DAY 1.0
#endif
#if SUNRAYS_AMOUNT_MAX_NIGHT == -1
#undef SUNRAYS_AMOUNT_MAX_NIGHT
										#define SUNRAYS_AMOUNT_MAX_NIGHT 1.0
#endif
#if SUNRAYS_BRIGHTNESS_INCREASE == -1
#undef SUNRAYS_BRIGHTNESS_INCREASE
										#define SUNRAYS_BRIGHTNESS_INCREASE 0.0
#endif





#if DOF_ENABLED == -1
#undef DOF_ENABLED
										#define DOF_ENABLED 0
#endif
#if DOF_SLOPE_TYPE == -1
#undef DOF_SLOPE_TYPE
										#define DOF_SLOPE_TYPE 1
#endif
#if DOF_NEAR_BLUR_START == -1
#undef DOF_NEAR_BLUR_START
										#define DOF_NEAR_BLUR_START 0.03
#endif
#if DOF_FAR_BLUR_START == -1
#undef DOF_FAR_BLUR_START
										#define DOF_FAR_BLUR_START 0.1
#endif
#if DOF_NEAR_BLUR_SLOPE == -1
#undef DOF_NEAR_BLUR_SLOPE
										#define DOF_NEAR_BLUR_SLOPE 0.05
#endif
#if DOF_FAR_BLUR_SLOPE == -1
#undef DOF_FAR_BLUR_SLOPE
										#define DOF_FAR_BLUR_SLOPE 0.3
#endif
#if DOF_NEAR_BLUR_STRENGTH == -1
#undef DOF_NEAR_BLUR_STRENGTH
										#define DOF_NEAR_BLUR_STRENGTH 0.8
#endif
#if DOF_FAR_BLUR_STRENGTH == -1
#undef DOF_FAR_BLUR_STRENGTH
										#define DOF_FAR_BLUR_STRENGTH 0.9
#endif
#if DOF_MIN_BLUR_STRENGTH == -1
#undef DOF_MIN_BLUR_STRENGTH
										#define DOF_MIN_BLUR_STRENGTH 0.0
#endif
#if DOF_NEAR_BLUR_SIZE == -1
#undef DOF_NEAR_BLUR_SIZE
										#define DOF_NEAR_BLUR_SIZE 2.0
#endif
#if DOF_FAR_BLUR_SIZE == -1
#undef DOF_FAR_BLUR_SIZE
										#define DOF_FAR_BLUR_SIZE 2.0
#endif
#if DOF_LOCKED_FOCAL_PLANE == -1
#undef DOF_LOCKED_FOCAL_PLANE
										#define DOF_LOCKED_FOCAL_PLANE 0
#endif
#if DOF_FOCAL_PLANE_DISTANCE == -1
#undef DOF_FOCAL_PLANE_DISTANCE
										#define DOF_FOCAL_PLANE_DISTANCE 10
#endif





#if MOTION_BLUR_ENABLED == -1
#undef MOTION_BLUR_ENABLED
										#define MOTION_BLUR_ENABLED 0
#endif
#if MOTION_BLUR_AMOUNT == -1
#undef MOTION_BLUR_AMOUNT
										#define MOTION_BLUR_AMOUNT 0.5
#endif





#if KUWAHARA_ENABLED == -1
#undef KUWAHARA_ENABLED
										#define KUWAHARA_ENABLED 0
#endif
#if KUWAHARA_SIZE == -1
#undef KUWAHARA_SIZE
										#define KUWAHARA_SIZE 1.0
#endif





#if SHARPEN_AMOUNT == -1
#undef SHARPEN_AMOUNT
										#define SHARPEN_AMOUNT 1.0
#endif
#if SHARPEN_VEL_ADDITION == -1
#undef SHARPEN_VEL_ADDITION
										#define SHARPEN_VEL_ADDITION 1.05
#endif
#if SHARPEN_DEPTH_ADDITION == -1
#undef SHARPEN_DEPTH_ADDITION
										#define SHARPEN_DEPTH_ADDITION 1.05
#endif
#if SHARPENING_DETECT_SIZE == -1
#undef SHARPENING_DETECT_SIZE
										#define SHARPENING_DETECT_SIZE 5
#endif





#if USE_GAMMA_CORRECTION == -1
#undef USE_GAMMA_CORRECTION
										#define USE_GAMMA_CORRECTION 0
#endif
#if GAMMA == -1
#undef GAMMA
										#define GAMMA 0
#endif
#if OVERWORLD_BRIGHTNESS == -1
#undef OVERWORLD_BRIGHTNESS
										#define OVERWORLD_BRIGHTNESS 1.05
#endif
#if NETHER_BRIGHTNESS == -1
#undef NETHER_BRIGHTNESS
										#define NETHER_BRIGHTNESS 0.95
#endif
#if END_BRIGHTNESS == -1
#undef END_BRIGHTNESS
										#define END_BRIGHTNESS 1.1
#endif
#if AUTO_EXPOSURE_ENABLED == -1
#undef AUTO_EXPOSURE_ENABLED
										#define AUTO_EXPOSURE_ENABLED 1
#endif
#if AUTO_EXPOSURE_BRIGHT_MULT == -1
#undef AUTO_EXPOSURE_BRIGHT_MULT
										#define AUTO_EXPOSURE_BRIGHT_MULT 1.1
#endif
#if AUTO_EXPOSURE_DARK_MULT == -1
#undef AUTO_EXPOSURE_DARK_MULT
										#define AUTO_EXPOSURE_DARK_MULT 1.9
#endif
#if VIBRANCE == -1
#undef VIBRANCE
										#define VIBRANCE 0.2
#endif
#if SATURATION == -1
#undef SATURATION
										#define SATURATION 0.2
#endif
#if SATURATION_DARK == -1
#undef SATURATION_DARK
										#define SATURATION_DARK 0.0
#endif
#if SATURATION_LIGHT == -1
#undef SATURATION_LIGHT
										#define SATURATION_LIGHT 0.0
#endif
#if SURFACE_CONTRAST == -1
#undef SURFACE_CONTRAST
										#define SURFACE_CONTRAST 0.6
#endif
#if UNDERGROUND_CONTRAST == -1
#undef UNDERGROUND_CONTRAST
										#define UNDERGROUND_CONTRAST 0.5
#endif
#if TONEMAPPER == -1
#undef TONEMAPPER
										#define TONEMAPPER 1
#endif





#if COLORBLIND_MODE == -1
#undef COLORBLIND_MODE
										#define COLORBLIND_MODE 0
#endif
#if COLORBLIND_INTENSITY == -1
#undef COLORBLIND_INTENSITY
										#define COLORBLIND_INTENSITY 1.0
#endif





#if VIGNETTE_ENABLED == -1
#undef VIGNETTE_ENABLED
										#define VIGNETTE_ENABLED 1
#endif
#if VIGNETTE_SCALE == -1
#undef VIGNETTE_SCALE
										#define VIGNETTE_SCALE 1.0
#endif
#if VIGNETTE_AMOUNT_SURFACE == -1
#undef VIGNETTE_AMOUNT_SURFACE
										#define VIGNETTE_AMOUNT_SURFACE 0.0
#endif
#if VIGNETTE_AMOUNT_UNDERGROUND == -1
#undef VIGNETTE_AMOUNT_UNDERGROUND
										#define VIGNETTE_AMOUNT_UNDERGROUND 0.5
#endif





#if HSV_POSTERIZE_ENABLED == -1
#undef HSV_POSTERIZE_ENABLED
										#define HSV_POSTERIZE_ENABLED 0
#endif
#if HSV_POSTERIZE_HUE_QUALITY == -1
#undef HSV_POSTERIZE_HUE_QUALITY
										#define HSV_POSTERIZE_HUE_QUALITY 16
#endif
#if HSV_POSTERIZE_SATURATION_QUALITY == -1
#undef HSV_POSTERIZE_SATURATION_QUALITY
										#define HSV_POSTERIZE_SATURATION_QUALITY 0
#endif
#if HSV_POSTERIZE_BRIGHTNESS_QUALITY == -1
#undef HSV_POSTERIZE_BRIGHTNESS_QUALITY
										#define HSV_POSTERIZE_BRIGHTNESS_QUALITY 16
#endif
#if HSV_POSTERIZE_STEP_SLOPE == -1
#undef HSV_POSTERIZE_STEP_SLOPE
										#define HSV_POSTERIZE_STEP_SLOPE 4.0
#endif





#if HEALTH_EFFECT_ENABLED == -1
#undef HEALTH_EFFECT_ENABLED
										#define HEALTH_EFFECT_ENABLED 1
#endif
#if HEALTH_EFFECT_STRENGTH == -1
#undef HEALTH_EFFECT_STRENGTH
										#define HEALTH_EFFECT_STRENGTH 1.5
#endif
#if DAMAGE_EFFECT_ENABLED == -1
#undef DAMAGE_EFFECT_ENABLED
										#define DAMAGE_EFFECT_ENABLED 1
#endif
#if DAMAGE_EFFECT_STRENGTH == -1
#undef DAMAGE_EFFECT_STRENGTH
										#define DAMAGE_EFFECT_STRENGTH 1.5
#endif
#if NEARBY_PARTICLE_TRANSPARENCY == -1
#undef NEARBY_PARTICLE_TRANSPARENCY
										#define NEARBY_PARTICLE_TRANSPARENCY 0.0
#endif
#if COLORED_PARTICLE_TRANSPARENCY == -1
#undef COLORED_PARTICLE_TRANSPARENCY
										#define COLORED_PARTICLE_TRANSPARENCY 0.0
#endif
#if ENTITY_FLASH_STRENGTH == -1
#undef ENTITY_FLASH_STRENGTH
										#define ENTITY_FLASH_STRENGTH 1.5
#endif





#if WAVING_ENABLED == -1
#undef WAVING_ENABLED
										#define WAVING_ENABLED 1
#endif
#if WAVING_SPEED == -1
#undef WAVING_SPEED
										#define WAVING_SPEED 1.2
#endif
#if WAVING_WORLD_SCALE == -1
#undef WAVING_WORLD_SCALE
										#define WAVING_WORLD_SCALE 0.3
#endif
#if WAVING_AMOUNT_1 == -1
#undef WAVING_AMOUNT_1
										#define WAVING_AMOUNT_1 0.3
#endif
#if WAVING_AMOUNT_2 == -1
#undef WAVING_AMOUNT_2
										#define WAVING_AMOUNT_2 0.7
#endif
#if WAVING_AMOUNT_3 == -1
#undef WAVING_AMOUNT_3
										#define WAVING_AMOUNT_3 1.3
#endif
#if WAVING_WEATHER_MULT == -1
#undef WAVING_WEATHER_MULT
										#define WAVING_WEATHER_MULT 2.5
#endif
#if WAVING_NIGHT_MULT == -1
#undef WAVING_NIGHT_MULT
										#define WAVING_NIGHT_MULT 0.7
#endif
#if HEIGHT_BASED_WAVING_ENABLED == -1
#undef HEIGHT_BASED_WAVING_ENABLED
										#define HEIGHT_BASED_WAVING_ENABLED 1
#endif





#if PHYSICALLY_WAVING_WATER_ENABLED == -1
#undef PHYSICALLY_WAVING_WATER_ENABLED
										#define PHYSICALLY_WAVING_WATER_ENABLED 1
#endif
#if PHYSICALLY_WAVING_WATER_AMOUNT_SURFACE == -1
#undef PHYSICALLY_WAVING_WATER_AMOUNT_SURFACE
										#define PHYSICALLY_WAVING_WATER_AMOUNT_SURFACE 1.5
#endif
#if PHYSICALLY_WAVING_WATER_AMOUNT_UNDERGROUND == -1
#undef PHYSICALLY_WAVING_WATER_AMOUNT_UNDERGROUND
										#define PHYSICALLY_WAVING_WATER_AMOUNT_UNDERGROUND 0.1
#endif
#if WAVING_WATER_SURFACE_ENABLED == -1
#undef WAVING_WATER_SURFACE_ENABLED
										#define WAVING_WATER_SURFACE_ENABLED 1
#endif
#if WAVING_WATER_SCALE == -1
#undef WAVING_WATER_SCALE
										#define WAVING_WATER_SCALE 1.0
#endif
#if WAVING_WATER_SPEED == -1
#undef WAVING_WATER_SPEED
										#define WAVING_WATER_SPEED 1.5
#endif
#if WAVING_WATER_FRESNEL_SURFACE == -1
#undef WAVING_WATER_FRESNEL_SURFACE
										#define WAVING_WATER_FRESNEL_SURFACE 0.75
#endif
#if WAVING_WATER_FRESNEL_UNDERGROUND == -1
#undef WAVING_WATER_FRESNEL_UNDERGROUND
										#define WAVING_WATER_FRESNEL_UNDERGROUND 0.6
#endif
#if WAVING_WATER_SURFACE_AMOUNT_SURFACE == -1
#undef WAVING_WATER_SURFACE_AMOUNT_SURFACE
										#define WAVING_WATER_SURFACE_AMOUNT_SURFACE 0.75
#endif
#if WAVING_WATER_SURFACE_AMOUNT_UNDERGROUND == -1
#undef WAVING_WATER_SURFACE_AMOUNT_UNDERGROUND
										#define WAVING_WATER_SURFACE_AMOUNT_UNDERGROUND 0.1
#endif





#if UNDERWATER_WAVINESS_ENABLED == -1
#undef UNDERWATER_WAVINESS_ENABLED
										#define UNDERWATER_WAVINESS_ENABLED 1
#endif
#if UNDERWATER_WAVINESS_AMOUNT == -1
#undef UNDERWATER_WAVINESS_AMOUNT
										#define UNDERWATER_WAVINESS_AMOUNT 1.0
#endif
#if UNDERWATER_WAVINESS_SCALE == -1
#undef UNDERWATER_WAVINESS_SCALE
										#define UNDERWATER_WAVINESS_SCALE 1.0
#endif
#if UNDERWATER_WAVINESS_SPEED == -1
#undef UNDERWATER_WAVINESS_SPEED
										#define UNDERWATER_WAVINESS_SPEED 1.0
#endif





#if WATER_TRANSPARENCY_SHALLOW == -1
#undef WATER_TRANSPARENCY_SHALLOW
										#define WATER_TRANSPARENCY_SHALLOW 0.75
#endif
#if WATER_TRANSPARENCY_DEEP == -1
#undef WATER_TRANSPARENCY_DEEP
										#define WATER_TRANSPARENCY_DEEP 0.4
#endif
#if WATER_FOAM_ENABLED == -1
#undef WATER_FOAM_ENABLED
										#define WATER_FOAM_ENABLED 1
#endif
#if WATER_FOAM_AMOUNT == -1
#undef WATER_FOAM_AMOUNT
										#define WATER_FOAM_AMOUNT 0.7
#endif
#if WATER_FOAM_SIZE == -1
#undef WATER_FOAM_SIZE
										#define WATER_FOAM_SIZE 1.5
#endif





#if WATER_TEXTURE_INFLUENCE == -1
#undef WATER_TEXTURE_INFLUENCE
										#define WATER_TEXTURE_INFLUENCE 0.9
#endif
#if WATER_BIOME_INFLUENCE == -1
#undef WATER_BIOME_INFLUENCE
										#define WATER_BIOME_INFLUENCE 1.2
#endif
#if WATER_TINT_SHALLOW_RED == -1
#undef WATER_TINT_SHALLOW_RED
										#define WATER_TINT_SHALLOW_RED 0.8
#endif
#if WATER_TINT_SHALLOW_GREEN == -1
#undef WATER_TINT_SHALLOW_GREEN
										#define WATER_TINT_SHALLOW_GREEN 1.1
#endif
#if WATER_TINT_SHALLOW_BLUE == -1
#undef WATER_TINT_SHALLOW_BLUE
										#define WATER_TINT_SHALLOW_BLUE 1.0
#endif
#if WATER_TINT_SHALLOW_BRIGHTNESS == -1
#undef WATER_TINT_SHALLOW_BRIGHTNESS
										#define WATER_TINT_SHALLOW_BRIGHTNESS 0.75
#endif
#if WATER_TINT_DEEP_RED == -1
#undef WATER_TINT_DEEP_RED
										#define WATER_TINT_DEEP_RED 0.8
#endif
#if WATER_TINT_DEEP_GREEN == -1
#undef WATER_TINT_DEEP_GREEN
										#define WATER_TINT_DEEP_GREEN 1.1
#endif
#if WATER_TINT_DEEP_BLUE == -1
#undef WATER_TINT_DEEP_BLUE
										#define WATER_TINT_DEEP_BLUE 1.0
#endif
#if WATER_TINT_DEEP_BRIGHTNESS == -1
#undef WATER_TINT_DEEP_BRIGHTNESS
										#define WATER_TINT_DEEP_BRIGHTNESS 0.75
#endif





#if HORIZON_FADE_STRENGTH == -1
#undef HORIZON_FADE_STRENGTH
										#define HORIZON_FADE_STRENGTH 1
#endif
#if SUN_BRIGHTNESS == -1
#undef SUN_BRIGHTNESS
										#define SUN_BRIGHTNESS 1.25
#endif
#if SUN_OPACITY == -1
#undef SUN_OPACITY
										#define SUN_OPACITY 3.0
#endif
#if MOON_BRIGHTNESS == -1
#undef MOON_BRIGHTNESS
										#define MOON_BRIGHTNESS 1.1
#endif
#if MOON_OPACITY == -1
#undef MOON_OPACITY
										#define MOON_OPACITY 1.0
#endif
#if SUN_MOON_WEATHER_OPACITY_DECREASE == -1
#undef SUN_MOON_WEATHER_OPACITY_DECREASE
										#define SUN_MOON_WEATHER_OPACITY_DECREASE 0.6
#endif
#if STARS_BRIGHTNESS == -1
#undef STARS_BRIGHTNESS
										#define STARS_BRIGHTNESS 2.5
#endif
#if DARKEN_STARS_NEAR_BLOCKLIGHT == -1
#undef DARKEN_STARS_NEAR_BLOCKLIGHT
										#define DARKEN_STARS_NEAR_BLOCKLIGHT 1
#endif
#if DARKENED_STARS_BRIGHTNESS == -1
#undef DARKENED_STARS_BRIGHTNESS
										#define DARKENED_STARS_BRIGHTNESS 0.5
#endif
#if WEATHER_TRANSPARENCY == -1
#undef WEATHER_TRANSPARENCY
										#define WEATHER_TRANSPARENCY 0.3
#endif
#if WEATHER_HORIZONTAL_AMOUNT == -1
#undef WEATHER_HORIZONTAL_AMOUNT
										#define WEATHER_HORIZONTAL_AMOUNT 0.6
#endif
#if SKY_WEATHER_BRIGHTNESS == -1
#undef SKY_WEATHER_BRIGHTNESS
										#define SKY_WEATHER_BRIGHTNESS 0.35
#endif
#if SKY_WEATHER_DESATURATION == -1
#undef SKY_WEATHER_DESATURATION
										#define SKY_WEATHER_DESATURATION 0.7
#endif





#if VANILLA_CLOUD_TRANSPARENCY == -1
#undef VANILLA_CLOUD_TRANSPARENCY
										#define VANILLA_CLOUD_TRANSPARENCY 0.3
#endif
#if NEARBY_CLOUD_TRANSPARENCY == -1
#undef NEARBY_CLOUD_TRANSPARENCY
										#define NEARBY_CLOUD_TRANSPARENCY 0.5
#endif
#if NEARBY_CLOUD_DIST == -1
#undef NEARBY_CLOUD_DIST
										#define NEARBY_CLOUD_DIST 30
#endif
#if CLOUD_FOG_START == -1
#undef CLOUD_FOG_START
										#define CLOUD_FOG_START 0.3
#endif
#if CLOUD_FOG_CURVE == -1
#undef CLOUD_FOG_CURVE
										#define CLOUD_FOG_CURVE 2
#endif
#if STORY_MODE_CLOUDS_ENABLED == -1
#undef STORY_MODE_CLOUDS_ENABLED
										#define STORY_MODE_CLOUDS_ENABLED 0
#endif
#if STORY_MODE_CLOUDS_CURVE == -1
#undef STORY_MODE_CLOUDS_CURVE
										#define STORY_MODE_CLOUDS_CURVE 2
#endif
#if REALISTIC_CLOUDS_ENABLED == -1
#undef REALISTIC_CLOUDS_ENABLED
										#define REALISTIC_CLOUDS_ENABLED 1
#endif
#if REALISTIC_CLOUD_DENSITY == -1
#undef REALISTIC_CLOUD_DENSITY
										#define REALISTIC_CLOUD_DENSITY 0.75
#endif
#if CLOUD_COVERAGE == -1
#undef CLOUD_COVERAGE
										#define CLOUD_COVERAGE 0.4
#endif
#if CLOUD_WEATHER_COVERAGE == -1
#undef CLOUD_WEATHER_COVERAGE
										#define CLOUD_WEATHER_COVERAGE 0.75
#endif
#if CLOUD_BOTTOM_Y == -1
#undef CLOUD_BOTTOM_Y
										#define CLOUD_BOTTOM_Y 120
#endif
#if CLOUD_TOP_Y == -1
#undef CLOUD_TOP_Y
										#define CLOUD_TOP_Y 250
#endif
#if CLOUD_OPACITY_DISTANCE == -1
#undef CLOUD_OPACITY_DISTANCE
										#define CLOUD_OPACITY_DISTANCE 56.0
#endif





#if CLOUD_DAY_RED == -1
#undef CLOUD_DAY_RED
										#define CLOUD_DAY_RED 0.9
#endif
#if CLOUD_DAY_GREEN == -1
#undef CLOUD_DAY_GREEN
										#define CLOUD_DAY_GREEN 0.95
#endif
#if CLOUD_DAY_BLUE == -1
#undef CLOUD_DAY_BLUE
										#define CLOUD_DAY_BLUE 1.0
#endif
#if CLOUD_NIGHT_RED == -1
#undef CLOUD_NIGHT_RED
										#define CLOUD_NIGHT_RED 0.15
#endif
#if CLOUD_NIGHT_GREEN == -1
#undef CLOUD_NIGHT_GREEN
										#define CLOUD_NIGHT_GREEN 0.3
#endif
#if CLOUD_NIGHT_BLUE == -1
#undef CLOUD_NIGHT_BLUE
										#define CLOUD_NIGHT_BLUE 0.65
#endif
#if CLOUD_SUNRISE_RED == -1
#undef CLOUD_SUNRISE_RED
										#define CLOUD_SUNRISE_RED 1.2
#endif
#if CLOUD_SUNRISE_GREEN == -1
#undef CLOUD_SUNRISE_GREEN
										#define CLOUD_SUNRISE_GREEN 0.65
#endif
#if CLOUD_SUNRISE_BLUE == -1
#undef CLOUD_SUNRISE_BLUE
										#define CLOUD_SUNRISE_BLUE 0.4
#endif
#if CLOUD_SUNSET_RED == -1
#undef CLOUD_SUNSET_RED
										#define CLOUD_SUNSET_RED 1.2
#endif
#if CLOUD_SUNSET_GREEN == -1
#undef CLOUD_SUNSET_GREEN
										#define CLOUD_SUNSET_GREEN 0.6
#endif
#if CLOUD_SUNSET_BLUE == -1
#undef CLOUD_SUNSET_BLUE
										#define CLOUD_SUNSET_BLUE 0.3
#endif
#if CLOUD_BOTTOM_TINT_RED == -1
#undef CLOUD_BOTTOM_TINT_RED
										#define CLOUD_BOTTOM_TINT_RED 0.6
#endif
#if CLOUD_BOTTOM_TINT_GREEN == -1
#undef CLOUD_BOTTOM_TINT_GREEN
										#define CLOUD_BOTTOM_TINT_GREEN 0.65
#endif
#if CLOUD_BOTTOM_TINT_BLUE == -1
#undef CLOUD_BOTTOM_TINT_BLUE
										#define CLOUD_BOTTOM_TINT_BLUE 1.1
#endif
#if CLOUD_WEATHER_TINT_RED == -1
#undef CLOUD_WEATHER_TINT_RED
										#define CLOUD_WEATHER_TINT_RED 0.65
#endif
#if CLOUD_WEATHER_TINT_GREEN == -1
#undef CLOUD_WEATHER_TINT_GREEN
										#define CLOUD_WEATHER_TINT_GREEN 0.7
#endif
#if CLOUD_WEATHER_TINT_BLUE == -1
#undef CLOUD_WEATHER_TINT_BLUE
										#define CLOUD_WEATHER_TINT_BLUE 0.9
#endif





#if CLOUD_LAYER_1_SCALE == -1
#undef CLOUD_LAYER_1_SCALE
										#define CLOUD_LAYER_1_SCALE 0.01
#endif
#if CLOUD_LAYER_1_WEIGHT == -1
#undef CLOUD_LAYER_1_WEIGHT
										#define CLOUD_LAYER_1_WEIGHT 1.0
#endif
#if CLOUD_LAYER_1_SPEED == -1
#undef CLOUD_LAYER_1_SPEED
										#define CLOUD_LAYER_1_SPEED 2.5
#endif
#if CLOUD_LAYER_2_SCALE == -1
#undef CLOUD_LAYER_2_SCALE
										#define CLOUD_LAYER_2_SCALE 0.04
#endif
#if CLOUD_LAYER_2_WEIGHT == -1
#undef CLOUD_LAYER_2_WEIGHT
										#define CLOUD_LAYER_2_WEIGHT 0.6
#endif
#if CLOUD_LAYER_2_SPEED == -1
#undef CLOUD_LAYER_2_SPEED
										#define CLOUD_LAYER_2_SPEED 1.4
#endif
#if CLOUD_LAYER_3_SCALE == -1
#undef CLOUD_LAYER_3_SCALE
										#define CLOUD_LAYER_3_SCALE 0.15
#endif
#if CLOUD_LAYER_3_WEIGHT == -1
#undef CLOUD_LAYER_3_WEIGHT
										#define CLOUD_LAYER_3_WEIGHT 0.2
#endif
#if CLOUD_LAYER_3_SPEED == -1
#undef CLOUD_LAYER_3_SPEED
										#define CLOUD_LAYER_3_SPEED 0.8
#endif
#if CLOUD_LAYER_4_SCALE == -1
#undef CLOUD_LAYER_4_SCALE
										#define CLOUD_LAYER_4_SCALE 0.6
#endif
#if CLOUD_LAYER_4_WEIGHT == -1
#undef CLOUD_LAYER_4_WEIGHT
										#define CLOUD_LAYER_4_WEIGHT 0.13
#endif
#if CLOUD_LAYER_4_SPEED == -1
#undef CLOUD_LAYER_4_SPEED
										#define CLOUD_LAYER_4_SPEED 0.3
#endif





#if NETHER_CLOUDS_ENABLED == -1
#undef NETHER_CLOUDS_ENABLED
										#define NETHER_CLOUDS_ENABLED 1
#endif
#if NETHER_CLOUDS_SCALE == -1
#undef NETHER_CLOUDS_SCALE
										#define NETHER_CLOUDS_SCALE 0.9
#endif
#if NETHER_CLOUDS_COVERAGE == -1
#undef NETHER_CLOUDS_COVERAGE
										#define NETHER_CLOUDS_COVERAGE 0.9
#endif
#if NETHER_CLOUDS_DENSITY == -1
#undef NETHER_CLOUDS_DENSITY
										#define NETHER_CLOUDS_DENSITY 0.5
#endif
#if NETHER_CLOUDS_TRANSPARENCY == -1
#undef NETHER_CLOUDS_TRANSPARENCY
										#define NETHER_CLOUDS_TRANSPARENCY 0.85
#endif
#if NETHER_CLOUDS_BASE_RED == -1
#undef NETHER_CLOUDS_BASE_RED
										#define NETHER_CLOUDS_BASE_RED 0.2
#endif
#if NETHER_CLOUDS_BASE_GREEN == -1
#undef NETHER_CLOUDS_BASE_GREEN
										#define NETHER_CLOUDS_BASE_GREEN 0.2
#endif
#if NETHER_CLOUDS_BASE_BLUE == -1
#undef NETHER_CLOUDS_BASE_BLUE
										#define NETHER_CLOUDS_BASE_BLUE 0.0
#endif
#if NETHER_CLOUDS_FOG_INFLUENCE == -1
#undef NETHER_CLOUDS_FOG_INFLUENCE
										#define NETHER_CLOUDS_FOG_INFLUENCE 0.75
#endif
#if NETHER_CLOUDS_TINT_RED == -1
#undef NETHER_CLOUDS_TINT_RED
										#define NETHER_CLOUDS_TINT_RED 1.0
#endif
#if NETHER_CLOUDS_TINT_GREEN == -1
#undef NETHER_CLOUDS_TINT_GREEN
										#define NETHER_CLOUDS_TINT_GREEN 1.0
#endif
#if NETHER_CLOUDS_TINT_BLUE == -1
#undef NETHER_CLOUDS_TINT_BLUE
										#define NETHER_CLOUDS_TINT_BLUE 1.0
#endif





#if END_CLOUDS_ENABLED == -1
#undef END_CLOUDS_ENABLED
										#define END_CLOUDS_ENABLED 1
#endif
#if END_CLOUDS_TYPE == -1
#undef END_CLOUDS_TYPE
										#define END_CLOUDS_TYPE 1
#endif
#if END_CLOUDS_SCALE == -1
#undef END_CLOUDS_SCALE
										#define END_CLOUDS_SCALE 1.6
#endif
#if END_CLOUDS_COVERAGE == -1
#undef END_CLOUDS_COVERAGE
										#define END_CLOUDS_COVERAGE 0.7
#endif
#if END_CLOUDS_TRANSPARENCY == -1
#undef END_CLOUDS_TRANSPARENCY
										#define END_CLOUDS_TRANSPARENCY 0.2
#endif
#if END_CLOUDS_RED_DARK == -1
#undef END_CLOUDS_RED_DARK
										#define END_CLOUDS_RED_DARK 0.15
#endif
#if END_CLOUDS_GREEN_DARK == -1
#undef END_CLOUDS_GREEN_DARK
										#define END_CLOUDS_GREEN_DARK 0.0
#endif
#if END_CLOUDS_BLUE_DARK == -1
#undef END_CLOUDS_BLUE_DARK
										#define END_CLOUDS_BLUE_DARK 0.2
#endif
#if END_CLOUDS_RED_BRIGHT == -1
#undef END_CLOUDS_RED_BRIGHT
										#define END_CLOUDS_RED_BRIGHT 1.25
#endif
#if END_CLOUDS_GREEN_BRIGHT == -1
#undef END_CLOUDS_GREEN_BRIGHT
										#define END_CLOUDS_GREEN_BRIGHT 0.7
#endif
#if END_CLOUDS_BLUE_BRIGHT == -1
#undef END_CLOUDS_BLUE_BRIGHT
										#define END_CLOUDS_BLUE_BRIGHT 1.0
#endif





#if SKY_DAY_RED == -1
#undef SKY_DAY_RED
										#define SKY_DAY_RED 0.3
#endif
#if SKY_DAY_GREEN == -1
#undef SKY_DAY_GREEN
										#define SKY_DAY_GREEN 0.4
#endif
#if SKY_DAY_BLUE == -1
#undef SKY_DAY_BLUE
										#define SKY_DAY_BLUE 0.95
#endif
#if SKY_DAY_BRIGHTNESS == -1
#undef SKY_DAY_BRIGHTNESS
										#define SKY_DAY_BRIGHTNESS 1.0
#endif
#if SKY_NIGHT_RED == -1
#undef SKY_NIGHT_RED
										#define SKY_NIGHT_RED 0.1
#endif
#if SKY_NIGHT_GREEN == -1
#undef SKY_NIGHT_GREEN
										#define SKY_NIGHT_GREEN 0.55
#endif
#if SKY_NIGHT_BLUE == -1
#undef SKY_NIGHT_BLUE
										#define SKY_NIGHT_BLUE 1.0
#endif
#if SKY_NIGHT_BRIGHTNESS == -1
#undef SKY_NIGHT_BRIGHTNESS
										#define SKY_NIGHT_BRIGHTNESS 1.0
#endif
#if SKY_HORIZON_DAY_RED == -1
#undef SKY_HORIZON_DAY_RED
										#define SKY_HORIZON_DAY_RED 0.45
#endif
#if SKY_HORIZON_DAY_GREEN == -1
#undef SKY_HORIZON_DAY_GREEN
										#define SKY_HORIZON_DAY_GREEN 0.5
#endif
#if SKY_HORIZON_DAY_BLUE == -1
#undef SKY_HORIZON_DAY_BLUE
										#define SKY_HORIZON_DAY_BLUE 1.0
#endif
#if SKY_HORIZON_DAY_BRIGHTNESS == -1
#undef SKY_HORIZON_DAY_BRIGHTNESS
										#define SKY_HORIZON_DAY_BRIGHTNESS 1.0
#endif
#if SKY_HORIZON_NIGHT_RED == -1
#undef SKY_HORIZON_NIGHT_RED
										#define SKY_HORIZON_NIGHT_RED 0.3
#endif
#if SKY_HORIZON_NIGHT_GREEN == -1
#undef SKY_HORIZON_NIGHT_GREEN
										#define SKY_HORIZON_NIGHT_GREEN 0.6
#endif
#if SKY_HORIZON_NIGHT_BLUE == -1
#undef SKY_HORIZON_NIGHT_BLUE
										#define SKY_HORIZON_NIGHT_BLUE 1.0
#endif
#if SKY_HORIZON_NIGHT_BRIGHTNESS == -1
#undef SKY_HORIZON_NIGHT_BRIGHTNESS
										#define SKY_HORIZON_NIGHT_BRIGHTNESS 1.3
#endif
#if SKY_HORIZON_SUNRISE_RED == -1
#undef SKY_HORIZON_SUNRISE_RED
										#define SKY_HORIZON_SUNRISE_RED 1.0
#endif
#if SKY_HORIZON_SUNRISE_GREEN == -1
#undef SKY_HORIZON_SUNRISE_GREEN
										#define SKY_HORIZON_SUNRISE_GREEN 0.6
#endif
#if SKY_HORIZON_SUNRISE_BLUE == -1
#undef SKY_HORIZON_SUNRISE_BLUE
										#define SKY_HORIZON_SUNRISE_BLUE 0.2
#endif
#if SKY_HORIZON_SUNRISE_BRIGHTNESS == -1
#undef SKY_HORIZON_SUNRISE_BRIGHTNESS
										#define SKY_HORIZON_SUNRISE_BRIGHTNESS 1.0
#endif
#if SKY_HORIZON_SUNSET_RED == -1
#undef SKY_HORIZON_SUNSET_RED
										#define SKY_HORIZON_SUNSET_RED 1.0
#endif
#if SKY_HORIZON_SUNSET_GREEN == -1
#undef SKY_HORIZON_SUNSET_GREEN
										#define SKY_HORIZON_SUNSET_GREEN 0.3
#endif
#if SKY_HORIZON_SUNSET_BLUE == -1
#undef SKY_HORIZON_SUNSET_BLUE
										#define SKY_HORIZON_SUNSET_BLUE 0.1
#endif
#if SKY_HORIZON_SUNSET_BRIGHTNESS == -1
#undef SKY_HORIZON_SUNSET_BRIGHTNESS
										#define SKY_HORIZON_SUNSET_BRIGHTNESS 1.0
#endif
#if SUNRISE_SUNSET_TOP_HEIGHT == -1
#undef SUNRISE_SUNSET_TOP_HEIGHT
										#define SUNRISE_SUNSET_TOP_HEIGHT 0.8
#endif
#if SUNRISE_SUNSET_BOTTOM_HEIGHT == -1
#undef SUNRISE_SUNSET_BOTTOM_HEIGHT
										#define SUNRISE_SUNSET_BOTTOM_HEIGHT 0.2
#endif





#if NETHER_SKY_BASE_RED == -1
#undef NETHER_SKY_BASE_RED
										#define NETHER_SKY_BASE_RED 0.15
#endif
#if NETHER_SKY_BASE_GREEN == -1
#undef NETHER_SKY_BASE_GREEN
										#define NETHER_SKY_BASE_GREEN 0.15
#endif
#if NETHER_SKY_BASE_BLUE == -1
#undef NETHER_SKY_BASE_BLUE
										#define NETHER_SKY_BASE_BLUE 0.15
#endif
#if NETHER_SKY_FOG_INFLUENCE == -1
#undef NETHER_SKY_FOG_INFLUENCE
										#define NETHER_SKY_FOG_INFLUENCE 0.9
#endif
#if NETHER_SKY_FOG_SATURATION == -1
#undef NETHER_SKY_FOG_SATURATION
										#define NETHER_SKY_FOG_SATURATION 0.9
#endif
#if NETHER_SKY_TINT_RED == -1
#undef NETHER_SKY_TINT_RED
										#define NETHER_SKY_TINT_RED 1.0
#endif
#if NETHER_SKY_TINT_GREEN == -1
#undef NETHER_SKY_TINT_GREEN
										#define NETHER_SKY_TINT_GREEN 1.0
#endif
#if NETHER_SKY_TINT_BLUE == -1
#undef NETHER_SKY_TINT_BLUE
										#define NETHER_SKY_TINT_BLUE 1.0
#endif





#if END_SKY_RED == -1
#undef END_SKY_RED
										#define END_SKY_RED 0.4
#endif
#if END_SKY_GREEN == -1
#undef END_SKY_GREEN
										#define END_SKY_GREEN 0.2
#endif
#if END_SKY_BLUE == -1
#undef END_SKY_BLUE
										#define END_SKY_BLUE 0.7
#endif
#if END_STATIC_RED == -1
#undef END_STATIC_RED
										#define END_STATIC_RED 1.25
#endif
#if END_STATIC_GREEN == -1
#undef END_STATIC_GREEN
										#define END_STATIC_GREEN 0.7
#endif
#if END_STATIC_BLUE == -1
#undef END_STATIC_BLUE
										#define END_STATIC_BLUE 0.9
#endif





#if UNDERGROUND_FOG_CONDITION == -1
#undef UNDERGROUND_FOG_CONDITION
										#define UNDERGROUND_FOG_CONDITION 1
#endif
#if UNDERGROUND_FOG_COLOR_TYPE == -1
#undef UNDERGROUND_FOG_COLOR_TYPE
										#define UNDERGROUND_FOG_COLOR_TYPE 2
#endif
#if UNDERGROUND_FOG_BRIGHTNESS == -1
#undef UNDERGROUND_FOG_BRIGHTNESS
										#define UNDERGROUND_FOG_BRIGHTNESS 0.3
#endif
#if END_FOG_RED == -1
#undef END_FOG_RED
										#define END_FOG_RED 0.4
#endif
#if END_FOG_GREEN == -1
#undef END_FOG_GREEN
										#define END_FOG_GREEN 0.2
#endif
#if END_FOG_BLUE == -1
#undef END_FOG_BLUE
										#define END_FOG_BLUE 0.5
#endif





#if BORDER_FOG_START_OVERWORLD == -1
#undef BORDER_FOG_START_OVERWORLD
										#define BORDER_FOG_START_OVERWORLD 0.7
#endif
#if BORDER_FOG_START_NETHER == -1
#undef BORDER_FOG_START_NETHER
										#define BORDER_FOG_START_NETHER 0.7
#endif
#if BORDER_FOG_START_END == -1
#undef BORDER_FOG_START_END
										#define BORDER_FOG_START_END 0.7
#endif
#if BORDER_FOG_END == -1
#undef BORDER_FOG_END
										#define BORDER_FOG_END 0.95
#endif
#if BORDER_FOG_CURVE_OVERWORLD == -1
#undef BORDER_FOG_CURVE_OVERWORLD
										#define BORDER_FOG_CURVE_OVERWORLD 2
#endif
#if BORDER_FOG_CURVE_NETHER == -1
#undef BORDER_FOG_CURVE_NETHER
										#define BORDER_FOG_CURVE_NETHER 1
#endif
#if BORDER_FOG_CURVE_END == -1
#undef BORDER_FOG_CURVE_END
										#define BORDER_FOG_CURVE_END 2
#endif





#if ATMOSPHERIC_FOG_DENSITY == -1
#undef ATMOSPHERIC_FOG_DENSITY
										#define ATMOSPHERIC_FOG_DENSITY 0.2
#endif
#if NIGHT_ATMOSPHERIC_FOG_DENSITY == -1
#undef NIGHT_ATMOSPHERIC_FOG_DENSITY
										#define NIGHT_ATMOSPHERIC_FOG_DENSITY 0.3
#endif
#if UNDERGROUND_FOG_DENSITY == -1
#undef UNDERGROUND_FOG_DENSITY
										#define UNDERGROUND_FOG_DENSITY 1.1
#endif
#if WEATHER_FOG_DENSITY == -1
#undef WEATHER_FOG_DENSITY
										#define WEATHER_FOG_DENSITY 3.0
#endif
#if PALE_GARDEN_FOG_DENSITY == -1
#undef PALE_GARDEN_FOG_DENSITY
										#define PALE_GARDEN_FOG_DENSITY 6.0
#endif
#if PALE_GARDEN_FOG_NIGHT_DENSITY == -1
#undef PALE_GARDEN_FOG_NIGHT_DENSITY
										#define PALE_GARDEN_FOG_NIGHT_DENSITY 100.0
#endif
#if BLINDNESS_EFFECT_FOG_DENSITY == -1
#undef BLINDNESS_EFFECT_FOG_DENSITY
										#define BLINDNESS_EFFECT_FOG_DENSITY 80.0
#endif
#if DARKNESS_EFFECT_FOG_DENSITY == -1
#undef DARKNESS_EFFECT_FOG_DENSITY
										#define DARKNESS_EFFECT_FOG_DENSITY 40.0
#endif
#if NETHER_FOG_DENSITY == -1
#undef NETHER_FOG_DENSITY
										#define NETHER_FOG_DENSITY 1.0
#endif
#if END_FOG_DENSITY == -1
#undef END_FOG_DENSITY
										#define END_FOG_DENSITY 0.3
#endif





#if WATER_FOG_BASE_RED == -1
#undef WATER_FOG_BASE_RED
										#define WATER_FOG_BASE_RED 0.05
#endif
#if WATER_FOG_BASE_GREEN == -1
#undef WATER_FOG_BASE_GREEN
										#define WATER_FOG_BASE_GREEN 0.0
#endif
#if WATER_FOG_BASE_BLUE == -1
#undef WATER_FOG_BASE_BLUE
										#define WATER_FOG_BASE_BLUE 0.2
#endif
#if WATER_VANILLA_FOG_INFLUENCE == -1
#undef WATER_VANILLA_FOG_INFLUENCE
										#define WATER_VANILLA_FOG_INFLUENCE 1.0
#endif
#if WATER_VANILLA_FOG_SATURATION == -1
#undef WATER_VANILLA_FOG_SATURATION
										#define WATER_VANILLA_FOG_SATURATION 1.0
#endif
#if WATER_FOG_TINT_RED == -1
#undef WATER_FOG_TINT_RED
										#define WATER_FOG_TINT_RED 1.0
#endif
#if WATER_FOG_TINT_GREEN == -1
#undef WATER_FOG_TINT_GREEN
										#define WATER_FOG_TINT_GREEN 1.3
#endif
#if WATER_FOG_TINT_BLUE == -1
#undef WATER_FOG_TINT_BLUE
										#define WATER_FOG_TINT_BLUE 0.7
#endif
#if WATER_FOG_DENSITY == -1
#undef WATER_FOG_DENSITY
										#define WATER_FOG_DENSITY 0.07
#endif
#if LAVA_FOG_RED == -1
#undef LAVA_FOG_RED
										#define LAVA_FOG_RED 0.9
#endif
#if LAVA_FOG_GREEN == -1
#undef LAVA_FOG_GREEN
										#define LAVA_FOG_GREEN 0.15
#endif
#if LAVA_FOG_BLUE == -1
#undef LAVA_FOG_BLUE
										#define LAVA_FOG_BLUE 0.0
#endif
#if LAVA_FOG_DENSITY == -1
#undef LAVA_FOG_DENSITY
										#define LAVA_FOG_DENSITY 4.0
#endif
#if POWDERED_SNOW_FOG_RED == -1
#undef POWDERED_SNOW_FOG_RED
										#define POWDERED_SNOW_FOG_RED 0.8
#endif
#if POWDERED_SNOW_FOG_GREEN == -1
#undef POWDERED_SNOW_FOG_GREEN
										#define POWDERED_SNOW_FOG_GREEN 1.0
#endif
#if POWDERED_SNOW_FOG_BLUE == -1
#undef POWDERED_SNOW_FOG_BLUE
										#define POWDERED_SNOW_FOG_BLUE 1.0
#endif
#if POWDERED_SNOW_FOG_DENSITY == -1
#undef POWDERED_SNOW_FOG_DENSITY
										#define POWDERED_SNOW_FOG_DENSITY 4.0
#endif





#if REFLECTIVE_EVERYTHING == -1
#undef REFLECTIVE_EVERYTHING
										#define REFLECTIVE_EVERYTHING 0
#endif
