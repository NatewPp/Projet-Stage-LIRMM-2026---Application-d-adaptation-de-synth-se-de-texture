...
float sampleNetherCloud(vec3 pos) {
	pos *= 0.125 / NETHER_CLOUDS_SCALE;
	pos.y *= 0.5;
	float cloudSample = 0.0;
	cloudSample += valueNoise((pos + frameTimeCounter / 8.0 ) * 1.0  ) * 1.0 ;
	cloudSample += valueNoise((pos + frameTimeCounter / 16.0) * 0.5  ) * 0.75;
	cloudSample += valueNoise((pos + frameTimeCounter / 32.0) * 0.25 ) * 0.5 ;
	cloudSample += valueNoise((pos + frameTimeCounter / 64.0) * 0.125) * 0.25;
	cloudSample /= 1.0 + 0.75 + 0.5 + 0.25;
	
	return percentThrough(cloudSample, 1.0 - NETHER_CLOUDS_COVERAGE * 0.75, 1.0);
}
...
vec2 computeNetherClouds(vec3 playerPos) {
	...
	float densityMult = length(stepVec) * NETHER_CLOUDS_DENSITY * -0.04;
